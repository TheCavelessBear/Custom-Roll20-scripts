const AoEBoom = (() => {
    ...
    function sendTemplateCard(template, caster, path) {
        ...
        if (template.type === 'wall') {
            applyButtons =
                '[Marked Gold Edge Is Harmful](!boom apply ' + path.id + ' --hazardSide top' + style +
                '[Opposite Edge Is Harmful](!boom apply ' + path.id + ' --hazardSide bottom' + style;
            orientation = 'Move and rotate the wall. The gold edge identifies the marked side.';
    ...
    function handleWall(msg, args) {
        ...
        syncWallMarker(template, wall.path);
...
function runFxApplyLine(line, caster, template) {
    ...
    function handlePathChange(path) {
        Object.keys(state.AoEBoom.templates).forEach(templateId => {
            ...
            if (template.pathId !== path.id) return;

            syncWallMarker(template, path);
        });
    }
