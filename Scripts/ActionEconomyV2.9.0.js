...
largeform: {
  ...
  function pruneActionEconomyState(options) {
    const removed = [];
    options = options || {};
    const dryRun = !!options.dryRun;
    const context = options.context || {};
    const missingGraphic = function(id) { return !!id && (id === context.graphicId || !getObj('graphic', id)); };
    const missingCharacter = function(id) { return !!id && (id === context.characterId || !getObj('character', id)); };
    const missingPlayer = function(id) { return !!id && (id === context.playerId || !getObj('player', id)); };
    const missingPage = function(id) { return !!id && !getObj('page', id); };
    const remove = function(store, id) { removed.push(store + '.' + id); if (!dryRun) delete S[store][id]; };
    const removeRecord = function(store, id) { removed.push(store + '.' + id); if (!dryRun) delete S[store][id]; };
    const hasMissingTokenReference = function(value, seen) {
      ...
      return Object.keys(value).some(function(key) {
        ...
        if (/^(?:caster|source|target|summon|owner|mount|dropped|control|visual)TokenId$/.test(key) && missingGraphic(entry)) return true;
        if (/^pageId$/.test(key) && missingPage(entry)) return true;
        // This is a placement-time snapshot, not a live token dependency.
        if (key === 'existingTokenIds') return false;
        if (Array.isArray(entry) && /Ids$/.test(key)) return entry.some(function(id) { return missingGraphic(id); });
        return hasMissingTokenReference(entry, seen);
      });
    };
    const removeStaleSourceFields = function(value, path, seen) {
      ...
      Object.keys(value).forEach(function(key) {
        const entry = value[key];
        if (key === context.graphicId) {
          removed.push(path + '.' + key);
          if (!dryRun) delete value[key];
          return;
        }
        if (/^(?:source|caster|target|owner|mount|dropped|summon|control|visual)TokenId$/.test(key) && missingGraphic(entry)) {
          removed.push(path + '.' + key);
          if (!dryRun) delete value[key];
          return;
        }
        if (entry && typeof entry === 'object') removeStaleSourceFields(entry, path + '.' + key, seen);
      });
    };
    const pruneSourceBoundMechanics = function(store) {
      Object.keys(S[store] || {}).forEach(function(tokenId) {
        const records = S[store][tokenId] || {};
        const target = getObj('graphic', tokenId);
        Object.keys(records).forEach(function(name) {
          const record = records[name];
          if (!record || !missingGraphic(record.sourceTokenId)) return;
          const concentration = (store === 'ongoingDamage' && record.duration === 'concentration') ||
            (store !== 'ongoingDamage' && record.durationOverride === 'concentration');
          if (!concentration) return;
          removed.push(store + '.' + tokenId + '.' + name);
          if (dryRun) return;
          if (target && store === 'effects') removeEffect(target, name);
          else if (target && store === 'conditions') removeCondition(target, name);
          else delete records[name];
        });
      });
    };
    const prunePendingSnapshots = function(value, path, seen) {
      if (!value || typeof value !== 'object') return;
      seen = seen || [];
      if (seen.indexOf(value) !== -1) return;
      seen.push(value);
      Object.keys(value).forEach(function(key) {
        const entry = value[key];
        if (key === 'existingTokenIds' && entry && typeof entry === 'object') {
          Object.keys(entry).forEach(function(tokenId) {
            if (missingGraphic(tokenId)) { removed.push(path + '.existingTokenIds.' + tokenId); if (!dryRun) delete entry[tokenId]; }
          });
          return;
        }
        if (entry && typeof entry === 'object') prunePendingSnapshots(entry, path + '.' + key, seen);
      });
    };
    const graphicStores = ['economy', 'attacksRemaining', 'movement', 'ignoreNextMove', 'movementLocked', 'effects', 'conditions', 'disarmedItems', 'droppedItemTokens', 'mounts', 'attributeModifiers', 'abilityScoreModifiers', 'aidHp', 'conditionLevels', 'economyLocks', 'ongoingDamage', 'summons', 'aoeControls', 'aoeHazards', 'aoeHazardTurnHits', 'tokenSizes', 'visualLinks', 'directionalHazards', 'directionalHazardTurnHits', 'difficultTerrain', 'terrainImmunities', 'hazardImmunities', 'damageSources', 'saveModifiers', 'telekinesis'];
    graphicStores.forEach(function(store) { Object.keys(S[store] || {}).forEach(function(id) { if (missingGraphic(id)) remove(store, id); }); });
    ['speeds', 'originalSpeeds'].forEach(function(store) { Object.keys(S[store] || {}).forEach(function(id) { if (missingGraphic(id) && missingCharacter(id)) remove(store, id); }); });
    ['attackCounts', 'features', 'auras', 'sheetCache', 'saveAdvantages'].forEach(function(store) { Object.keys(S[store] || {}).forEach(function(id) { if (missingCharacter(id)) remove(store, id); }); });
    ['pcCharacterIds', 'allyCharacterIds'].forEach(function(store) {
      const valid = (S[store] || []).filter(function(id) { const invalid = missingCharacter(id); if (invalid) removed.push(store + '.' + id); return !invalid; });
      if (!dryRun) S[store] = valid;
    });

    // These records describe relationships, so losing any participant invalidates
    // the record. Effects, conditions, and ongoing damage remain useful without a
    // source, so only their stale source fields are removed below.
    ['summons', 'aoeControls', 'aoeHazards', 'visualLinks', 'directionalHazards', 'droppedItemTokens', 'damageSources'].forEach(function(store) {
      Object.keys(S[store] || {}).forEach(function(id) {
        if (hasMissingTokenReference(S[store][id])) removeRecord(store, id);
      });
    });
    Object.keys(S.mounts || {}).forEach(function(riderId) {
      const record = S.mounts[riderId];
      if ((typeof record === 'string' && missingGraphic(record)) || (record && missingGraphic(record.mountId)) || hasMissingTokenReference(record)) removeRecord('mounts', riderId);
    });
    Object.keys(S.telekinesis || {}).forEach(function(casterId) {
      const record = S.telekinesis[casterId] || {};
      if (missingGraphic(record.casterTokenId || casterId)) { removeRecord('telekinesis', casterId); return; }
      if (missingGraphic(record.targetTokenId)) {
        removed.push('telekinesis.' + casterId + '.targetTokenId');
        if (!dryRun) { record.targetTokenId = null; record.targetType = null; record.restrained = false; }
      }
    });
    Object.keys(S.saveModifiers || {}).forEach(function(targetId) {
      const modifiers = S.saveModifiers[targetId] || {};
      Object.keys(modifiers).forEach(function(key) {
        if (hasMissingTokenReference(modifiers[key])) {
          removed.push('saveModifiers.' + targetId + '.' + key);
          if (!dryRun) delete modifiers[key];
        }
      });
      if (!Object.keys(modifiers).length) removeRecord('saveModifiers', targetId);
    });
    ['disarmedItems'].forEach(function(store) {
      Object.keys(S[store] || {}).forEach(function(ownerId) {
        const records = S[store][ownerId] || {};
        Object.keys(records).forEach(function(recordId) {
          if (hasMissingTokenReference(records[recordId])) {
            removed.push(store + '.' + ownerId + '.' + recordId);
            if (!dryRun) delete records[recordId];
          }
        });
        if (!Object.keys(records).length) removeRecord(store, ownerId);
      });
    });
    ['effects', 'conditions', 'ongoingDamage'].forEach(pruneSourceBoundMechanics);
    ['effects', 'conditions', 'ongoingDamage'].forEach(function(store) {
      Object.keys(S[store] || {}).forEach(function(id) { removeStaleSourceFields(S[store][id], store + '.' + id); });
    });
    Object.keys(S.aoeHazardTurnHits || {}).forEach(function(hazardId) {
      const hits = S.aoeHazardTurnHits[hazardId] || {};
      Object.keys(hits).forEach(function(targetId) {
        if (missingGraphic(targetId)) { removed.push('aoeHazardTurnHits.' + hazardId + '.' + targetId); if (!dryRun) delete hits[targetId]; }
      });
    });
    Object.keys(S.directionalHazardTurnHits || {}).forEach(function(hazardId) {
      const turns = S.directionalHazardTurnHits[hazardId] || {};
      Object.keys(turns).forEach(function(turnKey) {
        const hits = turns[turnKey] || {};
        Object.keys(hits).forEach(function(targetId) {
          if (missingGraphic(targetId)) { removed.push('directionalHazardTurnHits.' + hazardId + '.' + turnKey + '.' + targetId); if (!dryRun) delete hits[targetId]; }
        });
      });
    });
    ['pendingSummons', 'pendingVisualLinks', 'pendingDirectionalHazards'].forEach(function(store) {
      Object.keys(S[store] || {}).forEach(function(playerId) {
        const pending = S[store][playerId];
        if (playerId !== 'unknown' && playerId !== 'AoEBoom' && missingPlayer(playerId)) { remove(store, playerId); return; }
        if (Array.isArray(pending)) {
          const valid = pending.filter(function(record, index) {
            const invalid = !record || hasMissingTokenReference(record);
            if (invalid) removed.push(store + '.' + playerId + '.' + index);
            return !invalid;
          });
          if (!valid.length) { if (!dryRun) delete S[store][playerId]; }
          else if (!dryRun) S[store][playerId] = valid;
        } else if (!pending || hasMissingTokenReference(pending)) {
          remove(store, playerId);
        }
        if (S[store][playerId]) {
          prunePendingSnapshots(S[store][playerId], store + '.' + playerId);
          removeStaleSourceFields(S[store][playerId], store + '.' + playerId);
        }
      });
    });
    if (S.lastActiveTokenId && missingGraphic(S.lastActiveTokenId)) {
      removed.push('lastActiveTokenId');
      if (!dryRun) S.lastActiveTokenId = null;
    }
    return { removed: removed };
  }

  if (typeof PersistentStateManager !== 'undefined') PersistentStateManager.registerPruner('ActionEconomyV2', pruneActionEconomyState);
  ...
  function cleanupCharacterRegistryEntry(characterId) {
    ...
    delete S.features[characterId];
    delete S.auras[characterId];
    delete S.saveAdvantages[characterId];
    delete S.sheetCache[characterId];
    ['speeds', 'originalSpeeds'].forEach(storeName => {
      if (!getObj('graphic', characterId)) delete S[storeName][characterId];
    });
  }
  ...
  function cleanCharacterRegistries() {
    ...
    ["attackCounts", "features", "auras", "saveAdvantages", "sheetCache"].forEach(storeName => {
    ...
    ['speeds', 'originalSpeeds'].forEach(storeName => {
      Object.keys(S[storeName]).forEach(id => {
        if (getObj('graphic', id) || isExistingCharacterId(id)) return;
        delete S[storeName][id];
        removed++;
      });
    });
  ...
  async function applyTempHp(token, amount, label) {
    ...
    sendChat(
      ...
      "{{name=" + label + "}} " +
      "{{Target=" + tokenName(token) + "}} " +
      "{{Temp HP=" + currentTempHp + " â†’ " + tempHp + "}}"
    );
  }
  ...
  async function applyDarkOnesBlessing(token, reason) {
    ...
    applyTempHp(token, amount, "Dark Oneâ€™s Blessing â€” " + reason);
  }
  ...
  ActionEconomyV2API.processDamageResult = function(sourceTokenId, targetTokenId, hpBefore, hpAfter, details) {
    const target = getObj("graphic", targetTokenId);
...
function clearConcentrationSummons(casterToken) {
    ...
    Object.keys(S.effects).forEach(tokenId => {
      ...
      Object.keys(store).forEach(effectName => {
        ...
        if (
          ...
          (stored.durationOverride === "concentration" || (EFFECTS[effectName] && EFFECTS[effectName].concentration))
        ) {
  ...
  on("destroy:graphic", function (token) {
    ...
    pruneActionEconomyState({ dryRun: false, context: { graphicId: token.id } });
  });
  ...
  on("destroy:character", function (character) {
    ...
    pruneActionEconomyState({ dryRun: false, context: { characterId: character.id } });
  });
  ...
  on("chat:message", function (msg) {
    ...
    if (root === "!ae") {
      ...
      let mountSideOption = null;
      let mountSideOffset = null;

      if (command === "mount") {
        const sideOptionIndex = args.indexOf("--side");
        const sideOffsetIndex = args.indexOf("--side-offset");

        if (sideOptionIndex !== -1) {
          mountSideOption = args[sideOptionIndex + 1] || "";
        } else if (sideOffsetIndex !== -1) {
          const sideOffsetValue = args[sideOffsetIndex + 1];
          const cleanSideOffset = parseInt(sideOffsetValue, 10);

          if (!sideOffsetValue ||
            !/^[+-]?\d+$/.test(sideOffsetValue) ||
            !Number.isSafeInteger(cleanSideOffset)) {
            sendChat(AE, "/w gm Invalid mount side offset. Use --side-offset N with a signed whole number.");
            return;
          }

          mountSideOffset = cleanSideOffset;
        }
      }
      ...
      targetTokens.forEach(token => {
        ...
        if (command === "mount") {
          ...
          const mountedSide = mountSideOption !== null
            ? mountSideOption
            : mount && mountSideOffset !== null
              ? getCurrentTokenSideNumber(mount) + mountSideOffset
              : null;
