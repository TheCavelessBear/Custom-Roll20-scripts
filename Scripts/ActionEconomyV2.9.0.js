on("ready", function () {
  ...
  const EFFECTS = {
    ...
    shield: {
      ...
      duration: "startOfNextTurn",
      mechanical: "+5 AC. Immune to Magic Missile.",
      type: "effect",
      attributeModifier: {
        attrName: "ac",
        amount: 5
...
largeform: {
  ...
  log("=== ActionEconomyV2 2.9.0 Ready ===");
  ...
  Object.keys(S.tokenSizes).forEach(tokenId => {
    const storedTokenSizes = S.tokenSizes[tokenId];

    if (!storedTokenSizes || typeof storedTokenSizes !== "object") return;

    delete storedTokenSizes.largeform;

    if (!Object.keys(storedTokenSizes).length) {
      delete S.tokenSizes[tokenId];
    }
  });
  ...
  function normalizeSaveModifierKey(value) {
    return String(value || "")
      .toLowerCase()
      .replace(/[^a-z0-9]/g, "");
  }

  function getSaveModifierStore(token) {
    if (!S.saveModifiers[token.id]) {
      S.saveModifiers[token.id] = {};
    }

    return S.saveModifiers[token.id];
  }

  function applyNextSaveModifier(token, key, formula, sourceTokenId, label) {
    const cleanKey = normalizeSaveModifierKey(key);
    const cleanFormula = String(formula || "").toLowerCase().replace(/\s+/g, "");

    if (!token || !cleanKey || !cleanFormula.match(/^\d+d\d+$/)) {
      return false;
    }

    getSaveModifierStore(token)[cleanKey] = {
      key: cleanKey,
      label: String(label || key || "Saving Throw Modifier").replace(/_/g, " "),
      formula: cleanFormula,
      operation: "subtract",
      sourceTokenId: sourceTokenId || null,
      applied: Date.now(),
      expires: sourceTokenId ? "endOfSourceNextTurn" : "manual",
  ...
  function removeSaveModifier(token, key) {
    if (!token) return;

    const cleanKey = normalizeSaveModifierKey(key);
    const store = getSaveModifierStore(token);

    if (cleanKey === "all") {
      delete S.saveModifiers[token.id];
      return;
    }
  ...
  function consumeSaveModifiers(token) {
    const result = {
      total: 0,
      formula: "",
      notes: []
    ...
    Object.keys(store).forEach(key => {
      const modifier = store[key];

      if (!modifier || modifier.operation !== "subtract") return;

      const amount = rollDiceExpression(modifier.formula);

      if (!amount) return;

      result.total -= amount;
      result.formula += " - " + amount;
      result.notes.push(modifier.label + ": -" + amount + " (" + modifier.formula + ")");
  ...
  function clearSaveModifiersEndingOnSourceTurn(sourceToken) {
    Object.keys(S.saveModifiers).forEach(tokenId => {
      const target = getObj("graphic", tokenId);

      if (!target) {
        delete S.saveModifiers[tokenId];
        return;
      }

      const store = getSaveModifierStore(target);
      ...
      Object.keys(store).forEach(key => {
        ...
        if (
          !modifier ||
          modifier.expires !== "endOfSourceNextTurn" ||
          modifier.sourceTokenId !== sourceToken.id
        ) {
          return;
        }

        if (modifier.skipCurrentSourceTurnEnd) {
          modifier.skipCurrentSourceTurnEnd = false;
          return;
        }
  ...
  ActionEconomyV2API.consumeSaveModifiers = function(token) {
    return consumeSaveModifiers(token);
  };

  ActionEconomyV2API.applyNextSaveModifier = function(token, key, formula, sourceTokenId, label) {
    return applyNextSaveModifier(token, key, formula, sourceTokenId, label);
  };
  ...
  function registryAttackLines() {
    ...
    return characterIds
      .map(characterId =>
        registryCharacterName(characterId) + " â€” " + S.attackCounts[characterId] + " attacks " +
  ...
  function registryFeatureLines() {
    ...
    Object.keys(S.features).forEach(characterId => {
      ...
      lines.push(
        registryCharacterName(characterId) + " â€” " + names.join(", ") + " " +
  ...
  async function getDarkOnesBlessingAmount(token) {
    ...
    if (typeof getSheetItem !== "function") return 0;
  ...
  const pendingDamageNotifications = {};

  function cancelPendingDamageNotification(targetTokenId) {
    const pending = pendingDamageNotifications[targetTokenId];

    if (!pending) return;

    clearTimeout(pending.timeoutId);
    delete pendingDamageNotifications[targetTokenId];
  }

  function processDamageTaken(sourceTokenId, targetTokenId, damageTaken) {
    const target = getObj("graphic", targetTokenId);
    const amount = Math.max(0, parseInt(damageTaken, 10) || 0);

    if (!target || amount <= 0) return;

    cancelPendingDamageNotification(targetTokenId);

    if (sourceTokenId) {
      recordDamageSource(sourceTokenId, targetTokenId);
    }

    rollConcentrationSave(target, amount);
    processRepeatSaves(target, "damageTaken");
  }

  function queueFallbackDamageNotification(token, amount) {
    const damageTaken = Math.max(0, parseInt(amount, 10) || 0);

    if (!token || damageTaken <= 0) return;

    const existing = pendingDamageNotifications[token.id];
    ...
    pending.timeoutId = setTimeout(function() {
      const queued = pendingDamageNotifications[token.id];

      if (!queued) return;

      delete pendingDamageNotifications[token.id];
      processDamageTaken(null, token.id, queued.damageTaken);
    }, 100);

    pendingDamageNotifications[token.id] = pending;
  }
  ...
  async function applyDarkOnesBlessing(token, reason) {
    ...
    const amount = await getDarkOnesBlessingAmount(token);

    if (!amount || amount <= 0) return;

    await applyTempHp(token, amount, "Dark One’s Blessing — " + reason);
  }

  function reportDarkOnesBlessingFailure(token, error) {
    const name = token ? tokenName(token) : "an unknown token";

    log("ActionEconomyV2: Dark One’s Blessing failed for " + name + ". " + (error && error.message ? error.message : error));
    sendChat(AE, "/w gm ActionEconomyV2: Dark One’s Blessing could not be applied to " + name + ". Check Beacon access and the API sandbox log.");
  }

  const recentDeathTriggers = {};

  async function processDarkOnesBlessingDeathTrigger(deadToken) {
    try {
      if (!deadToken) return;
      ...
      if (
        recentDeathTriggers[deadToken.id] &&
        now - recentDeathTriggers[deadToken.id] < 1000
      ) {
        return;
      }

      recentDeathTriggers[deadToken.id] = now;
      ...
      if (
        ...
        try {
          await applyDarkOnesBlessing(sourceToken, "Kill");
        } catch (error) {
          reportDarkOnesBlessingFailure(sourceToken, error);
        }
      ...
      for (const token of tokens) {
        if (token.id === deadToken.id) continue;
        if (awarded[token.id]) continue;
        ...
        try {
          await applyDarkOnesBlessing(token, "Nearby Death");
          awarded[token.id] = true;
        } catch (error) {
          reportDarkOnesBlessingFailure(token, error);
        }
      }
    } catch (error) {
      reportDarkOnesBlessingFailure(null, error);
    } finally {
      if (deadToken) delete S.damageSources[deadToken.id];
    }
  }
  ...
  ActionEconomyV2API.processDamageResult = async function(sourceTokenId, targetTokenId, hpBefore, hpAfter, details) {
    try {
      const target = getObj("graphic", targetTokenId);
      ...
      const damageTaken = details.damageTaken !== undefined
        ? details.damageTaken
        : Math.max(0, (parseInt(hpBefore, 10) || 0) - (parseInt(hpAfter, 10) || 0));

      processDamageTaken(sourceTokenId, targetTokenId, damageTaken);

      if (hpBefore > 0 && hpAfter <= 0) {
        await processDarkOnesBlessingDeathTrigger(target);
      }
    } catch (error) {
      log("ActionEconomyV2: damage-result processing failed. " + (error && error.message ? error.message : error));
      sendChat(AE, "/w gm ActionEconomyV2: Damage-result processing failed. Check the API sandbox log.");
    }
  };

  ActionEconomyV2API.cancelPendingDamageNotification = function(targetTokenId) {
    cancelPendingDamageNotification(targetTokenId);
  };
...
ActionEconomyV2API.addPendingSummon = function(playerId, casterTokenId, summonName, concentration, count, timeoutSeconds, controlOptions, initiativeOptions) {
  ...
  async function rollRepeatSave(token, conditionName, repeatSave) {
    ...
    const saveModifier = consumeSaveModifiers(token);
    const total = d20 + saveBonus + auraBonus + saveModifier.total;
    ...
    if (success && repeatSave.endSourceConcentration && repeatSave.sourceTokenId) {
      const sourceToken = getObj("graphic", repeatSave.sourceTokenId);

      if (sourceToken && hasEffect(sourceToken, "concentrate")) {
        removeEffect(sourceToken, "concentrate");
      }
    }
    ...
    sendChat(
      AE,
      "&{template:default} " +
      "{{name=" + (repeatSave.label || "Repeat Save") + "}} " +
      "{{Target=" + tokenName(token) + "}} " +
      "{{Condition=" + registryDisplayName(CONDITIONS, conditionName) + "}} " +
      "{{Save=[[" + rollFormula + (auraBonus ? " + " + auraBonus : "") + saveModifier.formula + "]]}} " +
      (saveModifier.notes.length ? "{{Save Modifier=" + saveModifier.notes.join("<br>") + "}} " : "") +
      "{{DC=" + repeatSave.dc + "}} " +
  ...
  function rollConcentrationSave(token, damageTaken) {
    ...
    sendChat(
      ...
      "{{DC=" + dc + "}} " +
      "{{Saving Throw=[[" + rollFormula + (auraBonus ? " + " + auraBonus : "") + saveModifier.formula + "]]}} " +
      (saveModifier.notes.length ? "{{Save Modifier=" + saveModifier.notes.join("<br>") + "}} " : "") +
      "{{Result=" + (success ? "Success" : "Failure") + "}}"
    );
  }
  ...
  function forceMoveToken(sourceTokenId, targetTokenId, direction, feet) {
    ...
    if (normalizedDirection === "toward" || normalizedDirection === "away") {
      ...
      movementLabel = normalizedDirection + "Â°";
    }
  ...
  function getTelekinesisMovementVector(sourceToken, target, direction) {
    const normalizedDirection = normalizeForcedMovementDirection(direction);
    ...
    if (normalizedDirection === "toward" || normalizedDirection === "away") {
      ...
      return {
        x: towardX / length * multiplier,
        y: towardY / length * multiplier
      };
    }
  ...
  function getTelekinesisMovementDistance(sourceToken, target, direction, requestedFeet) {
    const requested = Math.min(30, Math.max(0, parseFloat(requestedFeet) || 0));
    const vector = getTelekinesisMovementVector(sourceToken, target, direction);

    if (!vector || requested <= 0) return 0;

    const pageScale = getPageScale(sourceToken);
    const pixelsPerFoot = 70 / pageScale;
    const radiusPixels = 60 * pixelsPerFoot;
    const dx = target.get("left") - sourceToken.get("left");
    const dy = target.get("top") - sourceToken.get("top");
    const b = 2 * (dx * vector.x + dy * vector.y);
    const c = dx * dx + dy * dy - radiusPixels * radiusPixels;
    const discriminant = b * b - 4 * c;

    if (discriminant < 0) return 0;

    const maxPixels = Math.max(0, (-b + Math.sqrt(discriminant)) / 2);
    const maxFeet = maxPixels / pixelsPerFoot;

    return Math.max(0, Math.min(requested, maxFeet));
  }

  function moveTelekinesisTarget(sourceToken, target, direction, requestedFeet) {
    const distance = getTelekinesisMovementDistance(
      sourceToken,
      target,
      direction,
      requestedFeet
    ...
    if (distance <= 0) {
      return {
        success: false,
        message: "The chosen movement would place the target outside the spell's 60-foot range."
      };
    }

    return forceMoveToken(sourceToken.id, target.id, direction, distance);
  }

  function removeTelekinesisRestraint(casterTokenId, targetTokenId) {
    const target = getObj("graphic", targetTokenId);

    if (!target) return;

    const restrained = getConditionStore(target).restrained;

    if (
      restrained &&
      restrained.sourceTokenId === casterTokenId
    ) {
      removeCondition(target, "restrained");
    }
  }

  function clearTelekinesisTarget(casterToken) {
    const record = S.telekinesis[casterToken.id];

    if (!record) return;

    if (record.targetTokenId) {
      removeTelekinesisRestraint(casterToken.id, record.targetTokenId);
    }

    record.targetTokenId = null;
    record.targetType = null;
    record.restrained = false;
  }

  function clearTelekinesis(casterToken, removeConcentration) {
    if (!casterToken) return;

    clearTelekinesisTarget(casterToken);
    delete S.telekinesis[casterToken.id];

    if (removeConcentration && hasEffect(casterToken, "concentrate")) {
      removeEffect(casterToken, "concentrate");
    }
  }

  function clearConcentrationTelekinesis(casterToken) {
    if (!casterToken || !S.telekinesis[casterToken.id]) return;

    clearTelekinesisTarget(casterToken);
    delete S.telekinesis[casterToken.id];
  }

  function spendTelekinesisAction(casterToken) {
    const economy = getEconomy(casterToken);

    if (!economy.action) {
      sendChat(AE, "/w gm " + tokenName(casterToken) + " has no Action available for Telekinesis.");
      return false;
    }

    economy.action = false;
    setAttacksRemaining(casterToken, 0);
    return true;
  }

  function beginTelekinesis(casterToken) {
    const existing = S.telekinesis[casterToken.id];

    if (existing) return existing;

    if (hasEffect(casterToken, "concentrate")) {
      removeEffect(casterToken, "concentrate");
    }

    applyEffect(casterToken, "concentrate");
    triggerEffectEvent(casterToken, "spell");

    S.telekinesis[casterToken.id] = {
    ...
    return S.telekinesis[casterToken.id];
  }

  function validateTelekinesisTokens(casterToken, target) {
    if (
      !casterToken ||
      !target ||
      casterToken.get("subtype") !== "token" ||
      target.get("subtype") !== "token"
    ) {
      return "Invalid caster or target token.";
    }

    if (casterToken.get("_pageid") !== target.get("_pageid")) {
      return "The caster and target must be on the same page.";
    }

    const distance = tokenDistanceFeet(casterToken, target);

    if (distance === null || distance > 60) {
      return "The target must be within 60 feet of the caster.";
    }

    return null;
  }

  async function useTelekinesisOnCreature(casterToken, target, direction, distance) {
    const validationError = validateTelekinesisTokens(casterToken, target);

    if (validationError) {
      sendChat(AE, "/w gm Telekinesis: " + validationError);
      return;
    }
    ...
    if (
      ...
      sendChat(AE, "/w gm Telekinesis requires SaveEffects 1.4 or later.");
      return;
    }

    if (!spendTelekinesisAction(casterToken)) return;

    const record = beginTelekinesis(casterToken);

    if (record.targetTokenId && record.targetTokenId !== target.id) {
      clearTelekinesisTarget(casterToken);
    }

    const result = await SaveEffectsAPI.rollSave({
      ...
      title: "Telekinesis",
      successText: "The target resists the spell.",
      failureText: "The target is moved and Restrained."
    });

    if (!result) return;

    if (result.success) {
      removeTelekinesisRestraint(casterToken.id, target.id);
      record.targetTokenId = null;
      record.targetType = null;
      record.restrained = false;
    } else {
      const movement = moveTelekinesisTarget(casterToken, target, direction, distance);

      applyCondition(target, "restrained", null, {
        durationOverride: "casterNextTurn",
        sourceTokenId: casterToken.id
      });
      enforceConditions(target);

      record.targetTokenId = target.id;
      record.targetType = "creature";
    ...
    sendChat(
      ...
      "{{Target=" + tokenName(target) + "}} " +
      "{{Result=" + (result.success ? "Save Successful" : "Save Failed") + "}} " +
      "{{Effect=" +
        (
          result.success
            ? "The target resists Telekinesis."
            : (
              result.movement && result.movement.success
                ? "Moved " + Math.round(result.movement.distance * 10) / 10 + " feet and Restrained until the end of the caster's next turn."
                : "Restrained until the end of the caster's next turn; movement could not be applied."
            )
        ) +
      "}}"
    );
  ...
  function useTelekinesisOnObject(casterToken, target, direction, distance, targetType) {
    const validationError = validateTelekinesisTokens(casterToken, target);
    ...
    if (!movement.success) {
      sendChat(AE, "/w gm Telekinesis: " + movement.message);
      return;
    }

    record.targetTokenId = target.id;
    record.targetType = targetType || "object";
    record.restrained = false;
    ...
    sendChat(
      ...
      "{{Caster=" + tokenName(casterToken) + "}} " +
      "{{Target=" + tokenName(target) + "}} " +
      "{{Effect=Moved " + Math.round(movement.distance * 10) / 10 + " feet.}}"
    );
  }

  async function useTelekinesisOnWornObject(casterToken, carrier, objectToken, direction, distance) {
    const carrierError = validateTelekinesisTokens(casterToken, carrier);
    const objectError = validateTelekinesisTokens(casterToken, objectToken);

    if (carrierError || objectError) {
      sendChat(AE, "/w gm Telekinesis: " + (carrierError || objectError));
      return;
    }
    ...
    if (record.targetTokenId && record.targetTokenId !== objectToken.id) {
      clearTelekinesisTarget(casterToken);
    }

    const result = await SaveEffectsAPI.rollSave({
      targetTokenId: carrier.id,
      saveKey: "str",
      dc: "spell",
      sourceTokenId: casterToken.id,
      conditionKey: "telekinesis",
      title: "Telekinesis",
      successText: "The carrier keeps hold of the object.",
      failureText: "The object is pulled free."
    });

    if (!result) return;

    let effectText = "The carrier keeps hold of the object.";

    if (result.failed) {
      const movement = moveTelekinesisTarget(casterToken, objectToken, direction, distance);

      if (movement.success) {
        record.targetTokenId = objectToken.id;
        record.targetType = "object";
        record.restrained = false;
        effectText = "The object is pulled free and moved " +
          Math.round(movement.distance * 10) / 10 +
          " feet.";
      } else {
        effectText = "The object is pulled free, but its token could not be moved.";
      }
    }
    ...
    sendChat(
      ...
      "&{template:default} " +
      "{{name=Telekinesis}} " +
      "{{Carrier=" + tokenName(carrier) + "}} " +
      "{{Object=" + tokenName(objectToken) + "}} " +
      "{{Result=" + (result.success ? "Save Successful" : "Save Failed") + "}} " +
      "{{Effect=" + effectText + "}}"
    );
  }

  function useTelekinesisFineControl(casterToken, target) {
    const validationError = validateTelekinesisTokens(casterToken, target);
    ...
    record.targetTokenId = target.id;
    record.targetType = "fine";
    record.restrained = false;

    sendChat(
      AE,
      "&{template:default} " +
      "{{name=Telekinesis â€” Fine Control}} " +
      "{{Caster=" + tokenName(casterToken) + "}} " +
      "{{Target=" + tokenName(target) + "}} " +
      "{{Effect=The caster performs fine telekinetic manipulation.}}"
    );
  }

  function enforceTelekinesisRange(changedToken) {
    Object.keys(S.telekinesis).forEach(casterTokenId => {
      const record = S.telekinesis[casterTokenId];

      if (
        !record ||
        (
          changedToken.id !== casterTokenId &&
          changedToken.id !== record.targetTokenId
        )
      ) {
        return;
      }
      ...
      const target = record.targetTokenId
        ? getObj("graphic", record.targetTokenId)
        : null;

      if (!casterToken) {
        delete S.telekinesis[casterTokenId];
        return;
      }
      ...
      if (distance !== null && distance <= 60) return;
      ...
      sendChat(
        ...
        "&{template:default} " +
        "{{name=Telekinesis}} " +
        "{{Effect=The controlled target left the spell's 60-foot range. Its active control effect ends.}}"
      );
    });
  }

  function handleTelekinesisCommand(args) {
    const action = String(args[1] || "").toLowerCase();
    const casterToken = getObj("graphic", args[2]);

    if (!casterToken || casterToken.get("subtype") !== "token") {
      sendChat(AE, "/w gm Telekinesis requires a valid caster token.");
      return;
    }

    if (action === "release") {
      clearTelekinesis(casterToken, true);
      sendChat(AE, "&{template:default} {{name=Telekinesis}} {{Effect=Telekinesis ends.}}");
      return;
    }

    if (action === "creature") {
      useTelekinesisOnCreature(
        casterToken,
        getObj("graphic", args[3]),
        args[4],
        args[5]
      );
      return;
    ...
    if (action === "worn") {
      useTelekinesisOnWornObject(
        casterToken,
        getObj("graphic", args[3]),
        getObj("graphic", args[4]),
        args[5],
        args[6]
      );
      return;
    ...
    if (action === "fine") {
      useTelekinesisFineControl(casterToken, getObj("graphic", args[3]));
      return;
    }

    sendChat(
      AE,
      "/w gm Format: !ae-telekinesis creature/object/worn/fine/release CASTER_ID ..."
    );
  }
  ...
  function showDifficultTerrainList() {
    ...
    const rows = terrainIds.map(id => {
      ...
      const detail = terrain.shape === "radius" ? " â€” " + terrain.radiusFeet + " ft" : "";

      return (terrain.name || "Difficult Terrain") + " â€” " + terrain.shape + detail + (token ? "" : " â€” missing token");
    });
  ...
  function getApplyOptions(args) {
    return {
      ...
      repeatSaveEndConcentration: getOptionValue(args, "--repeatSaveEndConcentration"),
      repeatSaveLabel: getOptionValue(args, "--repeatSaveLabel")
    };
  }
  ...
  function showAoeHazardList() {
    ...
    const rows = hazardIds.map(id => {
      ...
      const detail = shape === "radius" ? " â€” " + hazard.radius + " ft" : "";
...
function clearConcentrationSummons(casterToken) {
  ...
  function clearConcentrationEffects(token) {
    ...
    clearConcentrationTelekinesis(token);
  }
  ...
  function initializeActiveAttributeModifiers() {
    Object.keys(S.effects).forEach(tokenId => {
      const token = getObj("graphic", tokenId);

      if (!token) return;

      if (hasEffect(token, "haste")) {
        applyAttributeModifier(token, "haste", "ac", 2);
      }

      if (hasEffect(token, "shield")) {
        applyAttributeModifier(token, "shield", "ac", 5);
      }
    });
  }

  initializeActiveAttributeModifiers();
  ...
  function applyCondition(token, conditionName, level, options) {
    ...
    const repeatSaveEndConcentration =
      options &&
      (
        options.repeatSaveEndConcentration === true ||
        options.repeatSaveEndConcentration === "true" ||
        options.repeatSaveEndConcentration === "1" ||
        options.repeatSaveEndConcentration === "yes"
      );
    const repeatSaveLabel = options && options.repeatSaveLabel
      ? String(options.repeatSaveLabel).replace(/_/g, " ")
      : null;
    ...
    getConditionStore(token)[conditionName] = {
      ...
      repeatSave:
        ...
        !isNaN(repeatSaveDc) ?
        {
          ...
          sourceTokenId: sourceTokenId,
          endSourceConcentration: repeatSaveEndConcentration,
          label: repeatSaveLabel
        } :
        null,
      skipCurrentSourceTurnEnd:
        durationOverride === "casterNextTurn" &&
      ...
      .map(key => {
        ...
        if (key === "exhaustion") {
          ...
          if (level) {
            ...
            return display + " â€” " + exhaustionMechanicalText(level);
          }
        }
        ...
        return display + " â€” " + mechanical;
      })
      .join("<br>");
  }

  function publicRegistryListForToken(token, store, registry) {
    return Object.keys(store).map(key => {
      let display = registryDisplayName(registry, key);
  ...
  ActionEconomyV2API.getActiveConditions = function(token) {
    if (!token || token.get("subtype") !== "token") return [];

    return publicRegistryListForToken(
      token,
      getConditionStore(token),
      CONDITIONS
    );
  };

  ActionEconomyV2API.getActiveEffects = function(token) {
    if (!token || token.get("subtype") !== "token") return [];

    return publicRegistryListForToken(
      token,
      getEffectStore(token),
      EFFECTS
    );
  };

  ActionEconomyV2API.getTargetStatus = function(token) {
    return {
      conditions: ActionEconomyV2API.getActiveConditions(token),
      effects: ActionEconomyV2API.getActiveEffects(token)
    };
  };
    ...
    if (lastId) {
      const lastToken = getObj("graphic", lastId);
        if (lastToken) {
          ...
          clearSaveModifiersEndingOnSourceTurn(lastToken);
        }
    }
  ...
  on("change:graphic", function (token, prev) {
    ...
    enforceTelekinesisRange(token);
  });

  on("change:graphic:bar1_value", async function (token, prev) {
    if (!token || token.get("subtype") !== "token") return;
    ...
    queueFallbackDamageNotification(token, oldHp - newHp);

    if (oldHp > 0 && newHp <= 0) {
      await processDarkOnesBlessingDeathTrigger(token);
    }
  });
  ...
  on("change:graphic:bar2_value", function (token, prev) {
    ...
    queueFallbackDamageNotification(token, oldTempHp - newTempHp);
  });
  ...
  on("destroy:graphic", function (token) {
    ...
    if (S.telekinesis[token.id]) {
      clearTelekinesisTarget(token);
      delete S.telekinesis[token.id];
    }

    Object.keys(S.telekinesis).forEach(casterTokenId => {
      const record = S.telekinesis[casterTokenId];

      if (record && record.targetTokenId === token.id) {
        record.targetTokenId = null;
        record.targetType = null;
        record.restrained = false;
      }
    ...
    Object.keys(S.saveModifiers).forEach(targetTokenId => {
      const target = getObj("graphic", targetTokenId);
      ...
      Object.keys(store).forEach(key => {
        if (store[key] && store[key].sourceTokenId === token.id) {
          delete store[key];
        }
      });

      if (!Object.keys(store).length) {
        delete S.saveModifiers[targetTokenId];
      }
    });

    delete S.saveModifiers[token.id];
    cancelPendingDamageNotification(token.id);
  ...
  on("chat:message", function (msg) {
    ...
    if (root === "!ae-telekinesis") {
      handleTelekinesisCommand(args);
      return;
    }
    ...
    if (root === "!ae-savemod") {
      ...
      if (!token || token.get("subtype") !== "token") {
        sendChat(AE, "/w gm Invalid saving-throw modifier target.");
        return;
      }

      if (action === "add") {
        const key = args[3];
        const formula = args[4];
        const sourceTokenId = args[5] || null;
        const label = args.slice(6).join(" ") || key;

        if (!applyNextSaveModifier(token, key, formula, sourceTokenId, label)) {
          sendChat(AE, "/w gm Format: !ae-savemod add TARGET_ID KEY DICE SOURCE_ID [LABEL]");
          return;
        }
        ...
        sendChat(
          ...
          "{{Target=" + tokenName(token) + "}} " +
          "{{Effect=Subtract " + formula + " from the next saving throw.}} " +
          "{{Source=" + String(label || key).replace(/_/g, " ") + "}}"
        );
        return;
      }

      if (action === "remove") {
        removeSaveModifier(token, args[3] || "all");
        return;
      }

      sendChat(AE, "/w gm Format: !ae-savemod add/remove TARGET_ID KEY [DICE SOURCE_ID LABEL]");
      return;
    }
  ...
  ActionEconomyV2API.maintainState = function(options) {
    options = options || {};
    const dryRun = options.dryRun === true;
    const profile = options.profile || "prune";
    const result = { owner: AE, profile: profile, status: "ok", removed: 0, restored: 0, preserved: 0, warnings: [], actions: [], preservedDetails: [] };
    const supportedProfiles = ["prune", "combat", "scene", "setting", "campaign", "all"];
    const token = function(id) { return id ? getObj("graphic", id) : null; };
    const character = function(id) { return id ? getObj("character", id) : null; };
    const scheduledDeletes = [];
    const isScheduledForDelete = function(store, id) {
      return dryRun && scheduledDeletes.some(function(entry) { return entry.store === store && entry.id === id; });
    };
    const scheduleDelete = function(store, id) {
      if (dryRun && !isScheduledForDelete(store, id)) scheduledDeletes.push({ store: store, id: id });
    };
    const entries = function(store) {
      return store && typeof store === "object"
        ? Object.keys(store).filter(function(id) { return !isScheduledForDelete(store, id); })
        : [];
    };
    const action = function(text) { result.actions.push(text); };
    const remove = function(text, callback) {
      result.removed++;
      action(text);
      if (!dryRun && callback) callback();
    };
    const restore = function(text, callback) {
      result.restored++;
      action(text);
      if (!dryRun && callback) callback();
    };
    const preserve = function(text, warning) {
      result.preserved++;
      result.preservedDetails.push(text);
      if (warning) result.warnings.push(text);
    };
    const deleteEntry = function(store, id, text) {
      scheduleDelete(store, id);
      remove(text, function() { delete store[id]; });
    };
    const isRegisteredCharacter = function(characterId) {
      return (S.pcCharacterIds || []).includes(characterId) || (S.allyCharacterIds || []).includes(characterId);
    };
    const effectiveDuration = function(kind, name, entry) {
      const definition = kind === "effects" ? EFFECTS[name] : CONDITIONS[name];
      return (entry && entry.durationOverride) || (definition && definition.duration) || "manual";
    };
    const isCombatDuration = function(duration) {
      return ["combat", "endOfTurn", "startOfNextTurn", "casterNextTurn", "targetNextTurn"].includes(duration);
    };
    const clearStore = function(key, text) {
      const store = S[key] || {};
      entries(store).forEach(function(id) {
        deleteEntry(store, id, text + " " + id);
      });
    };
    const warnMissingRollback = function(kind, id) {
      preserve(kind + " rollback for missing live target " + id + " was preserved because reversal is not provable.", true);
    };

    if (!supportedProfiles.includes(profile)) {
      result.status = "invalid-profile";
      result.warnings.push("Unsupported ActionEconomyV2 maintenance profile: " + profile + ".");
      return result;
    }

    // Prune only stale state. Reversal records are deliberately retained when no
    // live target remains, because deleting them would make later restoration impossible.
    const tokenStores = [
      "economy", "speeds", "attacksRemaining", "movement", "ignoreNextMove",
      "movementLocked", "economyLocks", "conditionLevels", "saveModifiers",
      "damageSources", "aoeHazards", "visualLinks", "directionalHazards",
      "difficultTerrain"
    ];
    tokenStores.forEach(function(key) {
      const store = S[key] || {};
      entries(store).forEach(function(id) {
        if (!token(id)) deleteEntry(store, id, "prune " + key + " for missing token " + id);
      });
    });
    ["attributeModifiers", "abilityScoreModifiers", "aidHp", "tokenSizes", "disarmedItems", "mounts"].forEach(function(key) {
      const store = S[key] || {};
      entries(store).forEach(function(id) {
        const live = token(id);
        if (live && key !== "mounts") return;
        if (key === "mounts") {
          const record = store[id];
          const mountId = typeof record === "string" ? record : record && record.mountId;
          if (!live) {
            if (!mountId || !token(mountId)) deleteEntry(store, id, "prune mount relationship for missing rider " + id);
            else warnMissingRollback("mount", id);
          } else if (!mountId || !token(mountId)) {
            remove("release mount relationship with missing mount for " + id, function() { removeEffect(live, "mounted"); });
          }
          return;
        }
        warnMissingRollback(key, id);
      });
    });
    entries(S.aoeControls).forEach(function(controlId) {
      const control = token(controlId);
      const record = S.aoeControls[controlId] || {};
      const caster = record.casterTokenId ? token(record.casterTokenId) : null;
      if (control && (!record.casterTokenId || caster)) return;
      if (!control && record.casterTokenId && !caster) {
        deleteEntry(S.aoeControls, controlId, "prune AoE control with missing control and caster " + controlId);
        return;
      }
      scheduleDelete(S.aoeControls, controlId);
      remove("prune stale AoE control " + controlId, function() { clearAoe(["!ae-aoe", "clear", controlId]); });
    });
    entries(S.telekinesis).forEach(function(casterId) {
      const caster = token(casterId);
      if (caster) return;
      warnMissingRollback("telekinesis", casterId);
    });
    ["effects", "conditions"].forEach(function(kind) {
      const store = S[kind] || {};
      entries(store).forEach(function(tokenId) {
        if (!token(tokenId)) preserve(kind + " for missing token " + tokenId + " was preserved because reversal is not provable.", true);
      });
    });
    entries(S.summons).forEach(function(summonId) {
      const linked = S.summons[summonId] || {};
      if (!token(summonId)) {
        deleteEntry(S.summons, summonId, "prune stale summon link " + summonId);
      } else if (linked.casterTokenId && !token(linked.casterTokenId)) {
        preserve("summon " + summonId + " with missing caster " + linked.casterTokenId + " was preserved so later owner cleanup can remove the live summon.", true);
      }
    });
    entries(S.aoeHazards).forEach(function(hazardId) {
      const hazard = S.aoeHazards[hazardId] || {};
      if (!token(hazardId)) {
        deleteEntry(S.aoeHazards, hazardId, "prune AoE hazard for missing token " + hazardId);
        if (!dryRun && S.aoeHazardTurnHits) delete S.aoeHazardTurnHits[hazardId];
      } else if (hazard.sourceTokenId && !token(hazard.sourceTokenId)) {
        remove("prune stale AoE hazard source " + hazard.sourceTokenId, function() { delete hazard.sourceTokenId; });
      }
    });
    entries(S.directionalHazards).forEach(function(hazardId) {
      const hazard = S.directionalHazards[hazardId] || {};
      if (!token(hazardId)) {
        deleteEntry(S.directionalHazards, hazardId, "prune directional hazard for missing token " + hazardId);
        if (!dryRun && S.directionalHazardTurnHits) delete S.directionalHazardTurnHits[hazardId];
      } else {
        ["casterTokenId", "sourceTokenId"].forEach(function(field) {
          if (hazard[field] && !token(hazard[field])) {
            remove("prune stale directional hazard " + field + " " + hazard[field], function() { delete hazard[field]; });
          }
        });
      }
    });
    entries(S.visualLinks).forEach(function(visualId) {
      const link = S.visualLinks[visualId] || {};
      if (!token(visualId)) deleteEntry(S.visualLinks, visualId, "prune visual link for missing token " + visualId);
      else if (link.casterTokenId && !token(link.casterTokenId)) remove("prune stale visual caster " + link.casterTokenId, function() { delete link.casterTokenId; });
    });
    entries(S.droppedItemTokens).forEach(function(droppedId) {
      const link = S.droppedItemTokens[droppedId] || {};
      if (!token(droppedId)) {
        deleteEntry(S.droppedItemTokens, droppedId, "prune stale dropped-item link " + droppedId);
      } else if (!link.ownerTokenId || !token(link.ownerTokenId)) {
        preserve("dropped item " + droppedId + " with missing owner was preserved so later scene cleanup can remove the live owned token.", true);
      }
    });
    ["aoeHazardTurnHits", "directionalHazardTurnHits"].forEach(function(key) {
      const store = S[key] || {};
      const hazardStore = key === "aoeHazardTurnHits" ? S.aoeHazards : S.directionalHazards;
      entries(store).forEach(function(hazardId) {
        if (!hazardStore || !hazardStore[hazardId] || !token(hazardId)) {
          deleteEntry(store, hazardId, "prune stale " + key + " " + hazardId);
          return;
        }
        const ledger = store[hazardId] || {};
        entries(ledger).forEach(function(targetId) {
          if (!token(targetId)) {
            scheduleDelete(ledger, targetId);
            remove("prune stale hit-ledger target " + targetId, function() { delete ledger[targetId]; });
          }
        });
      });
    });
    entries(S.ongoingDamage).forEach(function(targetId) {
      const store = S.ongoingDamage[targetId] || {};
      if (!token(targetId)) {
        deleteEntry(S.ongoingDamage, targetId, "prune ongoing damage for missing token " + targetId);
        return;
      }
      entries(store).forEach(function(name) {
        const record = store[name] || {};
        if (record.sourceTokenId && !token(record.sourceTokenId)) remove("prune stale ongoing-damage source " + record.sourceTokenId, function() { delete record.sourceTokenId; });
      });
    });
    entries(S.damageSources).forEach(function(targetId) {
      const record = S.damageSources[targetId] || {};
      if (!token(targetId) || (record.sourceTokenId && !token(record.sourceTokenId))) deleteEntry(S.damageSources, targetId, "prune stale damage source " + targetId);
    });
    ["sheetCache", "attackCounts", "features", "auras", "saveAdvantages"].forEach(function(key) {
      const store = S[key] || {};
      entries(store).forEach(function(characterId) {
        if (!character(characterId)) deleteEntry(store, characterId, "prune " + key + " for missing character " + characterId);
      });
    });
    ["terrainImmunities", "hazardImmunities"].forEach(function(key) {
      const store = S[key] || {};
      entries(store).forEach(function(tokenId) {
        if (!token(tokenId)) deleteEntry(store, tokenId, "prune " + key + " for missing token " + tokenId);
      });
    });
    entries(S.originalSpeeds).forEach(function(id) {
      if (!token(id) && !character(id)) warnMissingRollback("speed", id);
    });
    ["pcCharacterIds", "allyCharacterIds"].forEach(function(key) {
      const list = Array.isArray(S[key]) ? S[key] : [];
      list.forEach(function(characterId) {
        if (!character(characterId)) remove("prune " + key + " entry for missing character " + characterId, function() { S[key] = S[key].filter(function(id) { return id !== characterId; }); });
      });
    });
    entries(S.pendingSummons).forEach(function(playerId) {
      const queue = Array.isArray(S.pendingSummons[playerId]) ? S.pendingSummons[playerId] : [S.pendingSummons[playerId]];
      const valid = queue.filter(function(pending) { return pending && (!pending.casterTokenId || !!token(pending.casterTokenId)); });
      if (valid.length !== queue.length) {
        if (!valid.length) scheduleDelete(S.pendingSummons, playerId);
        remove("prune stale pending summon records for " + playerId, function() { if (valid.length) S.pendingSummons[playerId] = valid; else delete S.pendingSummons[playerId]; });
      }
    });
    ["pendingVisualLinks", "pendingDirectionalHazards"].forEach(function(key) {
      entries(S[key]).forEach(function(playerId) {
        const pending = S[key][playerId];
        if (pending && pending.casterTokenId && !token(pending.casterTokenId)) deleteEntry(S[key], playerId, "prune stale " + key + " for " + playerId);
      });
    });
    if (profile === "prune") return result;

    const removeTimedEffectsAndConditions = function(removeAll) {
      ["effects", "conditions"].forEach(function(kind) {
        const store = S[kind] || {};
        entries(store).forEach(function(tokenId) {
          const live = token(tokenId);
          const records = store[tokenId] || {};
          entries(records).forEach(function(name) {
            const entry = records[name];
            const duration = effectiveDuration(kind, name, entry);
            const isExhaustionToKeep = kind === "conditions" && name === "exhaustion" &&
              isRegisteredCharacter(live && live.get("represents")) && (!removeAll || profile === "setting");
            if (!(removeAll || isCombatDuration(duration)) || isExhaustionToKeep) {
              preserve(kind + " " + name + " on " + tokenId + " remains for " + duration + " cleanup scope.");
              return;
            }
            if (!live) {
              preserve(kind + " " + name + " on missing token " + tokenId + " was preserved because reversal is not provable.", true);
              return;
            }
            scheduleDelete(records, name);
            remove("remove " + kind.slice(0, -1) + " " + name + " from " + tokenId, function() {
              if (kind === "effects") removeEffect(live, name);
              else removeCondition(live, name);
            });
          });
        });
      });
    };
    const deferredStrongCleanup = profile === "setting" || profile === "campaign" || profile === "all";
    const movementTokenIds = {};
    const rememberMovementTokens = function() {
      ["economy", "speeds", "attacksRemaining", "movement", "ignoreNextMove", "movementLocked", "economyLocks", "effects", "conditions"].forEach(function(key) {
        entries(S[key]).forEach(function(id) { movementTokenIds[id] = true; });
      });
      entries(S.mounts).forEach(function(riderId) {
        movementTokenIds[riderId] = true;
        const record = S.mounts[riderId];
        const mountId = typeof record === "string" ? record : record && record.mountId;
        if (mountId) movementTokenIds[mountId] = true;
      });
    };
    const clearRememberedMovementBars = function() {
      entries(movementTokenIds).forEach(function(tokenId) {
        const live = token(tokenId);
        if (live) remove("clear Bar 3 movement for " + tokenId, function() { clearMovementBar(live); });
      });
    };
    const clearCombatState = function() {
      removeTimedEffectsAndConditions(false);
      rememberMovementTokens();
      if (!deferredStrongCleanup) clearRememberedMovementBars();
      ["economy", "speeds", "attacksRemaining", "movement", "ignoreNextMove", "movementLocked", "economyLocks", "saveModifiers", "damageSources", "aoeHazardTurnHits", "directionalHazardTurnHits"].forEach(function(key) { clearStore(key, "clear combat " + key); });
      if (!deferredStrongCleanup) clearStore("sheetCache", "clear combat sheetCache");
      if (S.lastActiveTokenId !== null && S.lastActiveTokenId !== undefined) remove("clear last active token", function() { S.lastActiveTokenId = null; });
      entries(S.ongoingDamage).forEach(function(targetId) {
        const store = S.ongoingDamage[targetId] || {};
        entries(store).forEach(function(name) {
          if (store[name] && store[name].duration === "combat") {
            scheduleDelete(store, name);
            remove("remove combat ongoing damage " + name + " from " + targetId, function() { delete store[name]; });
          }
          else preserve("ongoing damage " + name + " on " + targetId + " remains for " + ((store[name] && store[name].duration) || "manual") + " cleanup scope.");
        });
        if (!dryRun && !entries(store).length) delete S.ongoingDamage[targetId];
      });
    };

    clearCombatState();
    if (profile === "combat") return result;

    entries(S.aoeControls).forEach(function(controlId) {
      scheduleDelete(S.aoeControls, controlId);
      remove("clear AoE control " + controlId, function() { clearAoe(["!ae-aoe", "clear", controlId]); });
    });
    entries(S.aoeHazards).forEach(function(hazardId) {
      const live = token(hazardId);
      scheduleDelete(S.aoeHazards, hazardId);
      remove("remove owned AoE hazard " + hazardId, function() { delete S.aoeHazards[hazardId]; delete S.aoeHazardTurnHits[hazardId]; if (live) live.remove(); });
    });
    entries(S.directionalHazards).forEach(function(hazardId) {
      const live = token(hazardId);
      scheduleDelete(S.directionalHazards, hazardId);
      remove("remove owned directional hazard " + hazardId, function() { delete S.directionalHazards[hazardId]; delete S.directionalHazardTurnHits[hazardId]; if (live) live.remove(); });
    });
    entries(S.visualLinks).forEach(function(visualId) {
      const live = token(visualId);
      scheduleDelete(S.visualLinks, visualId);
      remove("remove linked visual token " + visualId, function() { delete S.visualLinks[visualId]; if (live) live.remove(); });
    });
    ["pendingSummons", "pendingVisualLinks", "pendingDirectionalHazards"].forEach(function(key) { clearStore(key, "clear scene " + key); });
    entries(S.telekinesis).forEach(function(casterId) {
      const caster = token(casterId);
      if (!caster) { warnMissingRollback("telekinesis", casterId); return; }
      scheduleDelete(S.telekinesis, casterId);
      remove("clear telekinesis from " + casterId, function() { clearTelekinesis(caster, false); });
    });
    entries(S.disarmedItems).forEach(function(targetId) {
      const target = token(targetId);
      if (!target) { warnMissingRollback("disarm", targetId); return; }
      scheduleDelete(S.disarmedItems, targetId);
      remove("clear disarmed items from " + targetId, function() { removeCondition(target, "disarmed"); clearDisarmedItems(target); });
    });
    entries(S.droppedItemTokens).forEach(function(droppedId) {
      const dropped = token(droppedId);
      scheduleDelete(S.droppedItemTokens, droppedId);
      remove("remove dropped item token " + droppedId, function() { delete S.droppedItemTokens[droppedId]; if (dropped) dropped.remove(); });
    });
    clearStore("difficultTerrain", "clear difficult-terrain definition");
    if (profile === "scene") return result;

    removeTimedEffectsAndConditions(true);
    entries(S.ongoingDamage).forEach(function(targetId) {
      const store = S.ongoingDamage[targetId] || {};
      entries(store).forEach(function(name) {
        scheduleDelete(store, name);
        remove("clear ongoing damage " + name + " from " + targetId, function() { delete store[name]; });
      });
      if (!dryRun && !entries(store).length) delete S.ongoingDamage[targetId];
    });
    entries(S.summons).forEach(function(summonId) {
      const summon = token(summonId);
      scheduleDelete(S.summons, summonId);
      remove("remove owned summon " + summonId, function() { delete S.summons[summonId]; if (summon) summon.remove(); });
    });
    entries(S.mounts).forEach(function(riderId) {
      const rider = token(riderId);
      if (!rider) { warnMissingRollback("mount", riderId); return; }
      scheduleDelete(S.mounts, riderId);
      restore("release mount relationship for " + riderId, function() { releaseMountRelationship(rider); });
    });
    entries(S.originalSpeeds).forEach(function(id) {
      const speed = S.originalSpeeds[id];
      const live = token(id);
      const liveCharacter = character(id);
      const representative = liveCharacter ? findObjs({ type: "graphic", subtype: "token", represents: id })[0] : null;
      if (live || representative) {
        scheduleDelete(S.originalSpeeds, id);
        restore("restore original speed for " + id, function() { restoreSheetSpeed(live || representative); });
      } else if (liveCharacter) {
        scheduleDelete(S.originalSpeeds, id);
        restore("restore Beacon speed for " + id, function() { setSheetValue(id, "speed", speed); setCachedSheetValue(id, "speed", speed); delete S.originalSpeeds[id]; });
      } else warnMissingRollback("speed", id);
    });
    entries(S.attributeModifiers).forEach(function(tokenId) {
      const live = token(tokenId);
      if (!live) { warnMissingRollback("attribute modifier", tokenId); return; }
      entries(S.attributeModifiers[tokenId]).forEach(function(effectName) { scheduleDelete(S.attributeModifiers[tokenId], effectName); restore("restore attribute modifier " + effectName + " on " + tokenId, function() { removeAttributeModifier(live, effectName); }); });
    });
    entries(S.abilityScoreModifiers).forEach(function(tokenId) {
      const live = token(tokenId);
      if (!live) { warnMissingRollback("ability-score modifier", tokenId); return; }
      entries(S.abilityScoreModifiers[tokenId]).forEach(function(abilityName) {
        scheduleDelete(S.abilityScoreModifiers[tokenId], abilityName);
        restore("restore ability-score modifier " + abilityName + " on " + tokenId, function() { restoreAbilityScoreChange(live, abilityName); });
      });
    });
    entries(S.aidHp).forEach(function(tokenId) {
      const live = token(tokenId);
      if (!live) { warnMissingRollback("Aid HP", tokenId); return; }
      scheduleDelete(S.aidHp, tokenId);
      restore("restore Aid HP on " + tokenId, function() { removeAidHp(live); });
    });
    entries(S.tokenSizes).forEach(function(tokenId) {
      const live = token(tokenId);
      if (!live) { warnMissingRollback("token-size", tokenId); return; }
      entries(S.tokenSizes[tokenId]).forEach(function(effectName) { scheduleDelete(S.tokenSizes[tokenId], effectName); restore("restore token size " + effectName + " on " + tokenId, function() { removeTokenSizeModifier(live, effectName); }); });
    });
    ["terrainImmunities", "hazardImmunities"].forEach(function(key) {
      entries(S[key]).forEach(function(tokenId) {
        const live = token(tokenId);
        if (live && isRegisteredCharacter(live.get("represents"))) preserve(key + " on registered token " + tokenId + " remains for setting cleanup scope.");
        else deleteEntry(S[key], tokenId, "clear non-registration " + key + " " + tokenId);
      });
    });
    if (!dryRun) {
      ["effects", "conditions", "attributeModifiers", "abilityScoreModifiers", "aidHp", "tokenSizes", "ongoingDamage", "conditionLevels"].forEach(function(key) {
        entries(S[key]).forEach(function(id) {
          if (!entries(S[key][id]).length) delete S[key][id];
        });
      });
    }
    clearStore("sheetCache", "clear rebuilt-safe sheet cache");
    clearRememberedMovementBars();
    if (profile === "setting" || profile === "all") return result;

    // Campaign reset also clears the durable PC/Ally exhaustion retained by setting.
    removeTimedEffectsAndConditions(true);
    ["pcCharacterIds", "allyCharacterIds"].forEach(function(key) {
      const list = Array.isArray(S[key]) ? S[key].slice() : [];
      list.forEach(function(characterId) { remove("clear campaign " + key + " entry " + characterId, function() { S[key] = S[key].filter(function(id) { return id !== characterId; }); }); });
    });
    ["attackCounts", "features", "auras", "saveAdvantages", "conditionLevels", "terrainImmunities", "hazardImmunities"].forEach(function(key) { clearStore(key, "clear campaign " + key); });
    if (!dryRun) {
      S.disarmSequence = 0;
      S.lastActiveTokenId = null;
    }
