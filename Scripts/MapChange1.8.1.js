...
var MapChange = MapChange || (function() {
    ...
    var checkInstall = function() {
        ...
        state.MapChange.version = version;
        ...
        loadGlobalConfig();
        if (typeof PersistentStateManager !== 'undefined') {
            PersistentStateManager.registerPruner('MapChange', pruneMapChangeState);
        }
    };

    var pruneMapChangeState = function(options) {
        var root = state.MapChange || {};
        var removed = [];
        var dryRun = !!options.dryRun;
        var context = options.context || {};
        ['publicMaps', 'privateMaps', 'archiveMaps', 'hiddenMaps'].forEach(function(storeName) {
            var store = root[storeName] || {};
            Object.keys(store).forEach(function(name) {
                if (store[name] === context.pageId || !getObj('page', store[name])) { removed.push(storeName + '.' + name); if (!dryRun) delete store[name]; }
            });
        });
        if (Array.isArray(root.blockedPlayers)) {
            root.blockedPlayers.slice().forEach(function(playerId) {
                if (playerId === context.playerId || !getObj('player', playerId)) { removed.push('blockedPlayers.' + playerId); if (!dryRun) root.blockedPlayers.splice(root.blockedPlayers.indexOf(playerId), 1); }
            });
        }
    ...
    return {
        ...
        ,PruneState: pruneMapChangeState
    };
}());
...
on("ready", function() {
    ...
    on('destroy:page', function(page) { if (page) MapChange.PruneState({ dryRun: false, context: { pageId: page.id } }); });
    on('destroy:player', function(player) { if (player) MapChange.PruneState({ dryRun: false, context: { playerId: player.id } }); });
