...
const TokenTriggers = (() => {
    ...
    function resetBloodiedCombatRuntime(options) {
        options = options || {};
        const dryRun = options.dryRun === true;
        const preserveOnFailure = options.preserveOnFailure === true;
        const skipTokenIds = options.skipTokenIds || [];
        const summary = { removed: 0, restored: 0, preserved: 0, warnings: [] };

        Object.keys(state.TokenTriggers.tokens).forEach(function(tokenId) {
            if (skipTokenIds.indexOf(tokenId) !== -1) return;
            const runtime = state.TokenTriggers.tokens[tokenId];

            if (!runtime || !runtime.bloodied) return;

            const token = getObj('graphic', tokenId);
            const needsSideRestore = !!runtime.bloodied.originalSide;

            if (dryRun) {
                summary.removed++;
                if (needsSideRestore && token) summary.restored++;
                if (needsSideRestore && !token && preserveOnFailure) {
                    summary.removed--;
                    summary.preserved++;
                    summary.warnings.push('Bloodied presentation for missing token ' + tokenId + ' could not be reversed; runtime was preserved.');
                }
                return;
            }

            if (needsSideRestore && (!token || !restoreBloodiedTokenSide(token, runtime.bloodied))) {
                if (preserveOnFailure) {
                    summary.preserved++;
                    summary.warnings.push('Bloodied presentation for token ' + tokenId + ' could not be reversed; runtime was preserved.');
                    return;
                }
            } else if (needsSideRestore) {
                summary.restored++;
            }
    ...
    function resetRelentlessEnduranceCombatRuntime(options) {
        options = options || {};
        const dryRun = options.dryRun === true;
        const skipTokenIds = options.skipTokenIds || [];
    ...
    function applyBar1Change(token, characterId, oldHp, newHp) {
    ...
    function transitionMatches(record, oldHp, newHp) {
        if (!record) return false;
        if (record.processing) return true;

        const sameHp = function(left, right) {
            return left === right || (isNaN(left) && isNaN(right));
        };

        return (sameHp(oldHp, record.oldHp) &&
                (sameHp(newHp, record.requestedHp) || sameHp(newHp, record.finalHp))) ||
            (sameHp(oldHp, record.requestedHp) && sameHp(newHp, record.finalHp));
    }

    function processBar1Change(token, oldHpValue, newHpValue) {
        const oldHp = parseFloat(oldHpValue);
        const newHp = parseFloat(newHpValue);

        if (!token || typeof token.get !== 'function' || token.get('subtype') !== 'token') {
            return newHp;
        }

        const characterId = token.get('represents');

        if (!characterId || isNaN(newHp)) return newHp;

        const cached = bar1TransitionCache[token.id];

        if (transitionMatches(cached, oldHp, newHp)) {
            if (cached.processing) {
                const currentHp = parseFloat(token.get('bar1_value'));
                return isNaN(currentHp) ? newHp : currentHp;
            }

            return cached.finalHp;
        }
        ...
        bar1TransitionCache[token.id] = record;

        try {
            applyBar1Change(token, characterId, oldHp, newHp);
        } finally {
            const resolvedHp = parseFloat(token.get('bar1_value'));

            record.finalHp = isNaN(resolvedHp) ? newHp : resolvedHp;
            record.processing = false;

            setTimeout(function() {
                if (bar1TransitionCache[token.id] === record) {
                    delete bar1TransitionCache[token.id];
                }
            }, 0);
        }

        return record.finalHp;
    }

    function handleBar1Change(token, prev) {
        return processBar1Change(
            token,
            prev && prev.bar1_value,
            token && typeof token.get === 'function' ? token.get('bar1_value') : undefined
        );
    }
    ...
    TokenTriggersAPI = {
        processBar1Change: processBar1Change
    };
    ...
    return {
        ...
        maintainState: function(options) {
            ...
            const root = state.TokenTriggers || { characters: {}, tokens: {} };
            const result = { owner: SCRIPT, profile: profile, status: 'ok', removed: 0, restored: 0, preserved: 0, actions: [], preservedDetails: [], warnings: [] };
            const staleCharacters = Object.keys(root.characters || {}).filter(function(id) { return !getObj('character', id); });
            const staleTokens = Object.keys(root.tokens || {}).filter(function(id) { return !getObj('graphic', id); });
            const resetAllRuntime = profile === 'setting' || profile === 'campaign' || profile === 'all';

            result.removed += staleCharacters.length + staleTokens.length;
            if (!dryRun) {
                staleCharacters.forEach(function(id) { delete root.characters[id]; });
                staleTokens.forEach(function(id) { delete root.tokens[id]; });
            }
            if (staleCharacters.length || staleTokens.length) result.actions.push('prune ' + (staleCharacters.length + staleTokens.length) + ' missing character/token reference(s)');

            if (profile === 'prune') {
                result.preserved = Object.keys(root.characters || {}).length + Object.keys(root.tokens || {}).length - (dryRun ? staleCharacters.length + staleTokens.length : 0);
                if (result.preserved) result.preservedDetails.push('valid trigger configuration and token runtime');
                return result;
            }

            const bloodied = resetBloodiedCombatRuntime({ dryRun: dryRun, preserveOnFailure: true, skipTokenIds: staleTokens });
            const relentless = resetRelentlessEnduranceCombatRuntime({ dryRun: dryRun, skipTokenIds: staleTokens });
            result.removed += bloodied.removed + relentless.removed;
            result.restored += bloodied.restored;
            result.preserved += bloodied.preserved;
            result.warnings = result.warnings.concat(bloodied.warnings);
            if (bloodied.removed || relentless.removed) result.actions.push('clear Bloodied/Relentless Endurance combat runtime (' + (bloodied.removed + relentless.removed) + ')');

            if (resetAllRuntime) {
                Object.keys(root.tokens || {}).forEach(function(id) {
                    if (dryRun && staleTokens.indexOf(id) !== -1) return;
                    const token = getObj('graphic', id);
                    const runtime = root.tokens[id];
                    if (!runtime || !runtime.hpZero) return;
                    if (dryRun) {
                        result.removed++;
                        result.restored++;
                        return;
                    }
                    const outcome = restoreTokenPresentation(token, runtime.hpZero);
                    const hpZero = runtime.hpZero;
                    const restored = (!hpZero.originalSide || outcome.sideRestored) && outcome.layerRestored &&
                        outcome.sizeRestored && outcome.rotationRestored && outcome.bar1Restored;
                    if (!restored) {
                        result.preserved++;
                        result.warnings.push('HP-zero presentation for token ' + id + ' could not be fully restored; its runtime was preserved.');
                        return;
                    }
                    delete runtime.hpZero;
                    cleanTokenRuntime(id);
                    result.removed++;
                    result.restored++;
                });
                if (result.restored) result.actions.push('restore transient HP-zero token presentation before clearing runtime');
            } else {
                const hpZeroCount = Object.keys(root.tokens || {}).filter(function(id) {
                    return (!dryRun || staleTokens.indexOf(id) === -1) && root.tokens[id] && root.tokens[id].hpZero;
                }).length;
                result.preserved += hpZeroCount;
                if (hpZeroCount) result.preservedDetails.push('HP-zero runtime for creatures that remain defeated');
            }

            if (root.lastActiveTokenId) {
                result.removed++;
                result.actions.push('clear last active token runtime');
                if (!dryRun) root.lastActiveTokenId = null;
            }
            if (profile === 'campaign') {
                const characterCount = Object.keys(root.characters || {}).length;
                result.removed += characterCount;
                if (characterCount) result.actions.push('clear ' + characterCount + ' configured character trigger registration(s)');
                if (!dryRun) root.characters = {};
            } else {
                result.preserved += Object.keys(root.characters || {}).length - (dryRun ? staleCharacters.length : 0);
                result.preservedDetails.push('all configured character triggers');
            }
            return result;
        }
    };
