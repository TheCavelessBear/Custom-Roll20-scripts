...
function runFxApplyLine(line, caster, template) {
    ...
    function maintainState(options) {
        options = options || {};
        const dryRun = options.dryRun === true;
        const profile = options.profile || 'prune';
        const templates = (state.AoEBoom && state.AoEBoom.templates) || {};
        const result = { owner: BOOM, profile: profile, status: 'ok', removed: 0, restored: 0, preserved: 0, actions: [], preservedDetails: [], warnings: [] };
        const clearAll = profile !== 'prune';
        const ownedPathIds = {};
        Object.keys(templates).forEach(function(id) {
            const template = templates[id] || {};
            const stale = !getObj('path', template.pathId || id) || (template.casterTokenId && !getObj('graphic', template.casterTokenId));
            if (!clearAll && !stale) { result.preserved++; return; }
            result.removed++;
            if (getObj('path', template.pathId || id)) ownedPathIds[template.pathId || id] = true;
            if (template.markerPathId && getObj('path', template.markerPathId)) ownedPathIds[template.markerPathId] = true;
            if (!dryRun) handleClear({ content: '!boom clear ' + id, type: 'api' }, ['!boom', 'clear', id]);
        });
        result.restored = Object.keys(ownedPathIds).length;
        if (result.removed) result.actions.push((clearAll ? 'clear ' : 'prune ') + result.removed + ' template record(s) through AoEBoom cleanup');
        if (result.restored) result.actions.push('remove ' + result.restored + ' owned template/marker path object(s)');
        if (result.preserved) result.preservedDetails.push('valid unresolved templates');
        return result;
    }
