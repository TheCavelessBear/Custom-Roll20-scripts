...
function runFxApplyLine(line, caster, template) {
    ...
    function pruneAoEBoomState(options) {
        const templates = (state.AoEBoom || {}).templates || {};
        const removed = [];
        const dryRun = !!(options && options.dryRun);
        const context = options && options.context || {};
        Object.keys(templates).forEach(function(id) {
            const template = templates[id] || {};
            const invalid = id === context.pathId || template.pathId === context.pathId || template.casterTokenId === context.graphicId || template.sourceTokenId === context.graphicId || template.pageId === context.pageId || !getObj('path', template.pathId || id) || (template.casterTokenId && !getObj('graphic', template.casterTokenId)) || (template.sourceTokenId && !getObj('graphic', template.sourceTokenId)) || (template.pageId && !getObj('page', template.pageId));
            if (!invalid) return;
            removed.push('templates.' + id);
            if (!dryRun) delete templates[id];
        });
    ...
    on('ready', function() {
        checkInstall();
        if (typeof PersistentStateManager !== 'undefined') PersistentStateManager.registerPruner('AoEBoom', pruneAoEBoomState);
    });
    on('chat:message', handleInput);
    on('change:graphic', handleTokenChange);
    on('destroy:graphic', function(token) { if (token) pruneAoEBoomState({ dryRun: false, context: { graphicId: token.id } }); });
    on('destroy:path', function(path) { if (path) pruneAoEBoomState({ dryRun: false, context: { pathId: path.id } }); });
    on('destroy:page', function(page) { if (page) pruneAoEBoomState({ dryRun: false, context: { pageId: page.id } }); });
