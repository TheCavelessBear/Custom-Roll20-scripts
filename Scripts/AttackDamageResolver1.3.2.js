...
function pruneAttackDamageResolverState(options) {
    const root = state.AttackDamageResolver || {};
    const removed = [];
    const dryRun = !!(options && options.dryRun);
    const forcedId = options && options.context && options.context.graphicId;
    const missing = function(id) { return !!id && (id === forcedId || !getObj('graphic', id)); };
    const remove = function(parent, key, path) { removed.push(path); if (!dryRun) delete parent[key]; };
    Object.keys(root.attackTargets || {}).forEach(function(attackerId) {
        if (missing(attackerId) || missing(root.attackTargets[attackerId])) remove(root.attackTargets, attackerId, 'attackTargets.' + attackerId);
    });
    Object.keys(root.targetSlots || {}).forEach(function(slot) {
        const entry = root.targetSlots[slot] || {};
        if (missing(entry.attackerId) || missing(entry.targetId)) remove(root.targetSlots, slot, 'targetSlots.' + slot);
    });
    ['lastAttack', 'lastDamageUndo'].forEach(function(key) {
        const entry = root[key];
        if (entry && (missing(entry.attackerId) || missing(entry.targetId))) remove(root, key, key);
    });
    if (root.lastTurnTokenId && missing(root.lastTurnTokenId)) remove(root, 'lastTurnTokenId', 'lastTurnTokenId');
...
on('ready', function() {
    ...
    log('=== AttackDamageResolver 1.3.2 Ready ===');
    if (typeof PersistentStateManager !== 'undefined') PersistentStateManager.registerPruner('AttackDamageResolver', pruneAttackDamageResolverState);
});

on('destroy:graphic', function(token) {
    if (token) pruneAttackDamageResolverState({ dryRun: false, context: { graphicId: token.id } });
});
...
async function handleUncannyDodge(msg, args) {
    ...
    try {
        ...
        sendChat(
            'AttackDamageResolver',
            '&{template:default} ' +
            '{{name=' + target.get('name') + ' â€” Uncanny Dodge}} ' +
            '{{Reaction=The damage from the most recent attack is halved.}} ' +
            '{{Original Damage=' + originalDamage + '}} ' +
            '{{Damage Taken=' + reducedDamage + '}} ' +
            '{{HP=' + hpAfterFullDamage + ' â†’ ' + target.get('bar1_value') + '}} ' +
            '{{Temp HP=' + tempAfterFullDamage + ' â†’ ' + target.get('bar2_value') + '}}'
        );
    } catch (error) {
