...
const TokenTriggers = (() => {
    ...
    function handleCharacterDeleted(character) {
        ...
        pruneTokenTriggersState({ dryRun: false, context: { characterId: character.id } });
    }
    ...
    function handleTokenDeleted(token) {
        ...
        delete bar1TransitionCache[token.id];
        if (state.TokenTriggers.lastActiveTokenId === token.id) state.TokenTriggers.lastActiveTokenId = null;
        pruneTokenTriggersState({ dryRun: false, context: { graphicId: token.id } });
    }

    function pruneTokenTriggersState(options) {
        const root = state.TokenTriggers || {};
        const removed = [];
        const dryRun = !!options.dryRun;
        const context = options.context || {};
        ['characters', 'tokens'].forEach(function(storeName) {
            const type = storeName === 'characters' ? 'character' : 'graphic';
            const forcedId = storeName === 'characters' ? context.characterId : context.graphicId;
            Object.keys(root[storeName] || {}).forEach(function(id) { if (id === forcedId || !getObj(type, id)) { removed.push(storeName + '.' + id); if (!dryRun) delete root[storeName][id]; } });
        });
        if (root.lastActiveTokenId && (root.lastActiveTokenId === context.graphicId || !getObj('graphic', root.lastActiveTokenId))) {
            removed.push('lastActiveTokenId');
            if (!dryRun) root.lastActiveTokenId = null;
        }
        return { removed: removed };
    }
    on('ready', function() { checkInstall(); if (typeof PersistentStateManager !== 'undefined') PersistentStateManager.registerPruner('TokenTriggers', pruneTokenTriggersState); });
