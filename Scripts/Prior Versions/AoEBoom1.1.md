const AoEBoom = (() => {
    ...
    function tokenInsideWall(token, template) {
        ...
        const polygon = getPathWorldPoints(path);

        if (polygon.length < 3) return false;

        return getTokenSamplePoints(token).some(point => pointInsidePolygon(point, polygon));
    }
    ...
    function sendTemplateCard(template, caster, path) {
        ...
        const applyButton = '[Apply ' + template.title + '](!boom apply ' + path.id + style;
        ...
        sendChat(
            ...
            '{{Area=' + (template.type === 'wall' ? template.lengthFeet + ' ft × ' + template.widthFeet + ' ft wall' : template.radiusFeet + ' ft cone') + '}} ' +
            ...
            '{{Orientation=' + (template.type === 'wall' ? 'Move and rotate the wall template, then click Apply ' + template.title + '.' : 'Rotate your token to adjust the casting template.') + '}} ' +
            '{{Options=' + applyButton + ' ' + clearButton + '}}'
        );
    }
    ...
    function refreshTemplateFromPath(templateId) {
        ...
        template.origin = template.type === 'wall' && template.facingMode === 'template' ? {
            x: path.get('left'),
            y: path.get('top')
        } : {
            x: caster.get('left'),
        ...
        if (template.facingMode === 'caster') {
            ...
            state.AoEBoom.templates[template.pathId] = template;
...
function runFxApplyLine(line, caster, template) {
    ...
    function runSpawnApplyLine(line, template, path) {
        ...
        if (
            ...
            ActionEconomyV2API.addPendingDirectionalHazard(
                ...
                spawnName,
                {
                    ...
                    side: sideMatch ? String(sideMatch[1]).trim() : 'top',
                    rangeFeet: rangeMatch ? String(rangeMatch[1]).trim() : '10',
                    saveKey: template.saveKey,
                    dc: template.dc,
                    damageFormula: damageMatch ? String(damageMatch[1]).trim() : template.damageFormula,
                    damageType: typeMatch ? String(typeMatch[1]).trim() : template.damageType,
        ...
        SpawnDefaultToken.spawnAtXY({
            ...
            left: path ? path.get('left') : 0,
            top: path ? path.get('top') : 0,
            width: widthSquares * 70,
            height: heightSquares * 70,
            rotation: path ? (Number(path.get('rotation')) + 90) % 360 : 90
        });
    }
