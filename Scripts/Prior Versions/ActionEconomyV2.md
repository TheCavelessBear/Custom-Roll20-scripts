on("ready", function () {
  ...
  const EFFECTS = {
    ...
    haste: {
      ...
      duration: "combat",
      mechanical: "Speed doubled. Gain Haste Action.",
      type: "effect"
    },
...
largeform: {
  ...
  function getBeaconAttrName(attrName) {
    if (attrName === "isPC") return "user.isPC";
    if (attrName === "ae_attacks") return "user.ae_attacks";
    if (attrName === "ae_features") return "user.ae_features";
    if (attrName === "ae_auras") return "user.ae_auras";
    return attrName;
  }
  ...
  function refreshSheetValue(characterId, attrName) {
    ...
    const beaconAttrName = getBeaconAttrName(attrName);

    getSheetItem(characterId, beaconAttrName)
      .then(value => {
    ...
    refreshSheetValue(characterId, "isPC");
    refreshSheetValue(characterId, "ae_attacks");
    refreshSheetValue(characterId, "ac");
    refreshSheetValue(characterId, "constitution_save_bonus");
    refreshSheetValue(characterId, "ae_features");
    ...
    refreshSheetValue(characterId, "ae_auras");
  ...
  function applyAttributeModifier(token, effectName, attrName, amount) {
    const characterId = token.get("represents");
    if (!characterId) return;
  ...
  function addFeature(token, featureKey) {
    ...
    getFeatureStore(characterId)[featureKey] = true;
  }
  ...
  function removeFeature(token, featureKey) {
    ...
    delete getFeatureStore(characterId)[featureKey];
  }
  ...
  function hasFeature(token, featureKey) {
    ...
    return !!getFeatureStore(characterId)[featureKey];
  }
  ...
  function featureProvidesSaveAdvantage(token, saveKey, conditionKey) {
    ...
    const store = getFeatureStore(characterId);
  ...
  function parseAuraList(rawValue) {
    if (!rawValue) return [];
  ...
  function hasAura(token, auraKey) {
    ...
    const rawAuras = getCachedSheetValue(characterId, "ae_auras");
    return parseAuraList(rawAuras).includes(auraKey);
  }
  ...
  async function tokenHasDarkOnesBlessing(token) {
    ...
    if (hasFeature(token, "darkonesblessing")) return true;

    if (typeof getSheetItem !== "function") return false;

    try {
      const rawFeatures = await getSheetItem(characterId, "user.ae_features");
      const featureList = String(rawFeatures || "")
        .toLowerCase()
        .split(",")
        .map(s => s.trim())
        .filter(Boolean);

      if (featureList.includes("darkonesblessing")) {
        addFeature(token, "darkonesblessing");
        return true;
      }
    } catch (e) {
      return false;
  ...
  ActionEconomyV2API.processDamageResult = function(sourceTokenId, targetTokenId, hpBefore, hpAfter, damageTaken) {
    ...
    if (damageTaken && damageTaken > 0) {
      rollConcentrationSave(target, damageTaken);
    }
...
ActionEconomyV2API.addPendingSummon = function(playerId, casterTokenId, summonName, concentration, count, timeoutSeconds, controlOptions, initiativeOptions) {
  ...
  function initializePermanentFeatures(token) {
    ...
    const rawFeatures = getAttrValue(characterId, "ae_features");
    const featureList = rawFeatures ?
      String(rawFeatures).toLowerCase().split(",").map(s => s.trim()).filter(Boolean) :
      [];

    featureList.forEach(featureKey => {
      if (FEATURES[featureKey]) {
        addFeature(token, featureKey);
      }
    });

    const name = (token.get("name") || "").toLowerCase();

    if (!featureList.length && name.indexOf("valerius") !== -1) {
      addFeature(token, "eldritchmind");
    }

    if (!featureList.length && name.indexOf("huge-o") !== -1) {
      addFeature(token, "dangersense");
    }
  }
  ...
  function playRageEndEffects(token) {
    ...
    const attrValue = getAttrValue(characterId, "isPC");
    if (attrValue !== null) return String(attrValue).trim() === "1";
  ...
  function restoreSheetSpeed(token) {
    ...
    if (characterId) {
      const attrValue = getAttrValue(characterId, "ae_attacks");
      const sheetAttacks = parseInt(attrValue, 10);

      if (!isNaN(sheetAttacks) && sheetAttacks > 0) {
        return sheetAttacks;
      }
    }

    const key = getCharacterKey(token);
    return S.attackCounts[key] || 1;
  }

  function setAttackCount(token, count) {
    const key = getCharacterKey(token);
    S.attackCounts[key] = count;
  }
  ...
  function cleanDirectionalSide(side) {
    ...
    if (["top", "bottom", "left", "right", "inside", "outside"].includes(value)) {
      return value;
    }
  ...
  function addPendingDirectionalHazard(playerId, casterTokenId, hazardName, options) {
    ...
    S.pendingDirectionalHazards[playerId || "AoEBoom"] = {
      ...
      rangeFeet: (function() {
        const parsedRange = parseFloat(options.rangeFeet);
        return isNaN(parsedRange) ? 10 : parsedRange;
      })(),
      radiusFeet: (function() {
        const parsedRadius = parseFloat(options.radiusFeet);
        return isNaN(parsedRadius) ? null : parsedRadius;
      })(),
      damageFormula: options.damageFormula || "5d8",
      damageType: options.damageType || "Fire",
      saveKey: String(options.saveKey || "dex").toLowerCase(),
      dc: options.dc === undefined || options.dc === null ? "spell" : String(options.dc),
      successMode: String(options.successMode || "half").toLowerCase(),
  ...
  function tokenInsideDirectionalHazardZone(token, hazardToken, hazard) {
    ...
    const rangePixels = (typeof hazard.rangeFeet === "number" ? hazard.rangeFeet : 10) * 70 / scale;
    ...
    if (hazard.shape === "ring") {
      const radiusPixels = typeof hazard.radiusFeet === "number" ?
        hazard.radiusFeet * 70 / scale :
        Math.min(halfWidth, halfHeight);
      const distance = Math.sqrt(point.x * point.x + point.y * point.y);

      if (hazard.side === "inside") {
        return distance <= radiusPixels;
      }

      if (hazard.side === "outside") {
        return distance >= radiusPixels &&
          distance <= radiusPixels + rangePixels;
      }
  ...
  async function castAoe(args, playerId) {
    const name = args[2];
    ...
    const damageType = args[8];
    const successMode = args[9];

    const casterToken = getObj("graphic", casterTokenId);

    if (!name || !casterToken || isNaN(radius) || !normalizeSaveKey(saveKey) || !dcInput || !damageFormula || !damageType || !successMode) {
      sendChat(AE, "/w gm Format: !ae-aoe cast NAME CASTER_ID RADIUS SAVE DC DAMAGE TYPE half/none --instant true/false");
      return;
    }
    ...
    if (!character) {
      sendChat(AE, "/w gm No character named AoEControlToken found.");
      return;
    }

    character.get("defaulttoken", function(defaultToken) {
      if (!defaultToken) {
        sendChat(AE, "/w gm AoEControlToken has no default token.");
        return;
      }
...
function clearConcentrationSummons(casterToken) {
  ...
  function mountCreature(rider, mount) {
  ...
  function getMountedCreature(rider) {
    const mountId = S.mounts[rider.id];
    if (!mountId) return null;

    const mount = getObj("graphic", mountId);
  ...
  function canDismount(rider) {
    return hasEnoughMovement(rider, getHalfSpeedCost(rider));
  }
  ...
  function effectMenuTemplate() {
    return (
      ...
      "{{Combat Buffs=" + registryButtonList("!ae-effect", ["reckless", "sacred", "divine", "nature", "vengeblade"], EFFECTS) + "}} " +
  ...
  function aeDebugMenuTemplate() {
    return (
      ...
      "{{Token Info=[Features](!ae features) [Creature Type](!ae type)}} " +
  ...
  function handleTurnChange() {
    ...
    refreshSheetCache(activeToken);
    resetMovement(activeToken);
    setAttacksRemaining(activeToken, getAttackCount(activeToken));

    if (isPCToken(activeToken)) {
      startTurn(activeToken);
    }
  }
  ...
  on("chat:message", function (msg) {
    ...
    if (root === "!ae") {
      ...
      if (command === "attacks") {
        if (!selected.length || isNaN(amount)) return;
        selected.forEach(t => setAttackCount(t, amount));
        sendChat(AE, "/w gm Saved attack count " + amount + ".");
        return;
      }
    ...
    if (root === "!ae-aoe") {
      ...
      sendChat(AE, "/w gm Format: !ae-aoe cast/trigger/clear");
      return;
    }
