        ...
        try {
            ...
            if (UDL && lightRad !== -999) {
                ...
                if (lightDim !== "" && iLightDim > 0) {
                    ...
                    baseObj.low_light_distance = iLightDim.toString();
                }
            ...
            if (spawnObj && UDL) {
                ...
                const reportLighting = (stage) => {
                    const spawnedToken = getObj('graphic', spawnedTokenId);
                    
                    if (!spawnedToken) {
                        sendChat('SpawnAPI', `/w gm ${stage}: token unavailable`);
                        return;
                    }
                    ...
                    sendChat(
                        'SpawnAPI',
                        `/w gm ${stage}: ` +
                        `input=${lightRad}/${lightDim}; ` +
                        `base=${baseObj.bright_light_distance}/${baseObj.low_light_distance}; ` +
                        `token=${spawnedToken.get('bright_light_distance')}/${spawnedToken.get('low_light_distance')}`
                    );
                };
                
                reportLighting('Immediate');
                setTimeout(() => reportLighting('250ms'), 250);
                setTimeout(() => reportLighting('1000ms'), 1000);
                setTimeout(() => reportLighting('3000ms'), 3000);
            }
