const pruneExecutionerState = function(options) {
    const removed = [];
    const dryRun = !!(options && options.dryRun);
    const graphicId = options && options.context && options.context.graphicId;
    Object.keys(state.Executioner || {}).forEach(function(tokenId) {
        if (tokenId === graphicId || !getObj('graphic', tokenId)) { removed.push(tokenId); if (!dryRun) delete state.Executioner[tokenId]; }
    });
...
on('ready', function() {
    ...
    if (typeof PersistentStateManager !== 'undefined') PersistentStateManager.registerPruner('Executioner', pruneExecutionerState);
});

on('destroy:graphic', function(token) { if (token) pruneExecutionerState({ dryRun: false, context: { graphicId: token.id } }); });
