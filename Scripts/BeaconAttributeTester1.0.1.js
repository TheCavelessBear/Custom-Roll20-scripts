const BeaconAttributeTester = (() => {
    ...
    on('ready', function() {
        ...
        log('=== BeaconAttributeTester 1.0.1 Ready ===');
        if (typeof PersistentStateManager !== 'undefined') PersistentStateManager.registerPruner('BeaconAttributeTester', pruneBeaconAttributeTesterState);
        log('Commands: !btest read ATTR | !btest snapshot ATTR | !btest compare ATTR | !btest write ATTR VALUE --confirm');
    });

    function pruneBeaconAttributeTesterState(options) {
        const snapshots = (state.BeaconAttributeTester || {}).snapshots || {};
        const removed = [];
        const dryRun = !!options.dryRun;
        const context = options.context || {};
        Object.keys(snapshots).forEach(function(key) {
            const entry = snapshots[key] || {};
            if (entry.playerId === context.playerId || entry.characterId === context.characterId || (entry.playerId && !getObj('player', entry.playerId)) || (entry.characterId && !getObj('character', entry.characterId))) { removed.push('snapshots.' + key); if (!dryRun) delete snapshots[key]; }
        });
        return { removed: removed };
    }

    on('destroy:player', function(player) { if (player) pruneBeaconAttributeTesterState({ dryRun: false, context: { playerId: player.id } }); });
    on('destroy:character', function(character) { if (character) pruneBeaconAttributeTesterState({ dryRun: false, context: { characterId: character.id } }); });
