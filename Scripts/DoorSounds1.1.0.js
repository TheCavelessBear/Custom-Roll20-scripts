const DoorSounds = (() => {
    ...
    function maintainState(options) {
        ...
        const result = { owner: SCRIPT, profile: options.profile || 'prune', status: 'ok', removed: 0, restored: 0, preserved: 0, actions: [], preservedDetails: [], warnings: [] };
        const root = state.DoorSounds;
        if (!root || !root.doors) return result;
        const doorIds = Object.keys(root.doors);
        const staleIds = doorIds.filter(function(doorId) { return !getObj('door', doorId); });
        result.removed = staleIds.length;
        result.preserved = doorIds.length - staleIds.length;
        if (!dryRun) staleIds.forEach(function(doorId) { delete root.doors[doorId]; });
        if (result.removed) result.actions.push('prune ' + result.removed + ' deleted-door assignment(s)');
        result.preservedDetails.push('sound groups, configuration, and valid door assignments');
        return result;
    }
