on('ready', function() {
    ...
    log('=== SaveEffects 1.3.2 Ready ===');
    if (typeof PersistentStateManager !== 'undefined') PersistentStateManager.registerPruner('SaveEffects', pruneSaveEffectsState);
});

function pruneSaveEffectsState(options) {
    const root = state.SaveEffects || {};
    const removed = [];
    const dryRun = !!(options && options.dryRun);
    const context = options && options.context || {};
    Object.keys(root.sources || {}).forEach(function(playerId) {
        const tokenId = root.sources[playerId];
        if (playerId === context.playerId || tokenId === context.graphicId || !getObj('player', playerId) || !getObj('graphic', tokenId)) {
            removed.push('sources.' + playerId);
            if (!dryRun) delete root.sources[playerId];
        }
    });
...
on('destroy:graphic', function(token) {
    if (token) pruneSaveEffectsState({ dryRun: false, context: { graphicId: token.id } });
});

on('destroy:player', function(player) {
    if (player) pruneSaveEffectsState({ dryRun: false, context: { playerId: player.id } });
});
