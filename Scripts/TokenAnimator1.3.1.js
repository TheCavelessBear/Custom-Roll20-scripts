...
const TokenAnimator = (() => {
    ...
    function checkInstall() {
        ...
        if (state.TokenSizeAnimator) {
            const legacyTokens = state.TokenSizeAnimator.tokens || {};
            Object.keys(legacyTokens).forEach(function(tokenId) {
                if (!state.TokenAnimator.tokens[tokenId]) {
                    state.TokenAnimator.tokens[tokenId] = legacyTokens[tokenId];
                }
            });
            delete state.TokenSizeAnimator;
        }
    ...
    function pruneTokenAnimatorState(options) {
        const removed = [];
        const dryRun = !!options.dryRun;
        const graphicId = options.context && options.context.graphicId;
        const tokens = (state.TokenAnimator || {}).tokens || {};
        Object.keys(tokens).forEach(function(tokenId) {
            if (tokenId === graphicId || !getObj('graphic', tokenId)) { removed.push('tokens.' + tokenId); if (!dryRun) delete tokens[tokenId]; }
        });
        if (state.TokenSizeAnimator) { removed.push('TokenSizeAnimator'); if (!dryRun) delete state.TokenSizeAnimator; }
    ...
    on('ready', function() { checkInstall(); if (typeof PersistentStateManager !== 'undefined') PersistentStateManager.registerPruner('TokenAnimator', pruneTokenAnimatorState); });
    on('chat:message', handleInput);
    on('destroy:graphic', function(token) {
        handleTokenDestroyed(token);
        if (token) pruneTokenAnimatorState({ dryRun: false, context: { graphicId: token.id } });
    });
