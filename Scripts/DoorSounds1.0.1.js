const DoorSounds = (() => {
    ...
    function pruneDoorSoundsState(options) {
        const doors = (state.DoorSounds || {}).doors || {};
        const removed = [];
        const dryRun = !!options.dryRun;
        const doorId = options.context && options.context.doorId;
        Object.keys(doors).forEach(function(id) {
            if (id === doorId || !getObj('door', id)) { removed.push('doors.' + id); if (!dryRun) delete doors[id]; }
        });
        return { removed: removed };
    }
    on('ready', function() { checkInstall(); if (typeof PersistentStateManager !== 'undefined') PersistentStateManager.registerPruner('DoorSounds', pruneDoorSoundsState); });
    on('chat:message', handleInput);
    on('change:door:isOpen', handleDoorChange);
    on('destroy:door', function(door) { if (door) pruneDoorSoundsState({ dryRun: false, context: { doorId: door.id } }); });
