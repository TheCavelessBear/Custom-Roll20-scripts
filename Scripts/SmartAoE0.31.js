const SmartAoE = (() => {
    ...
    */
    ...
    const maintainState = function(options) {
        options = options || {};
        const dryRun = options.dryRun === true;
        const profile = options.profile || 'prune';
        const root = state[scriptName] || {};
        const links = Array.isArray(root.links) ? root.links.slice() : [];
        const result = { owner: scriptName, profile: profile, status: 'ok', removed: 0, restored: 0, preserved: 0, actions: [], preservedDetails: [], warnings: [] };
        const clearScene = profile === 'scene' || profile === 'setting' || profile === 'campaign' || profile === 'all';
        const ownedArtifactIds = {};
        const flattenPathIds = function(value) {
            return (Array.isArray(value) ? value : []).reduce(function(ids, entry) {
                return ids.concat(Array.isArray(entry) ? flattenPathIds(entry) : [entry]);
            }, []).filter(Boolean);
        };
        const isStale = function(link) {
            if (!link || !link.controlTokID || !link.originTokID) return true;
            const pathIds = flattenPathIds(link.pathIDs);
            return !getObj('graphic', link.controlTokID) || !getObj('graphic', link.originTokID) ||
                !pathIds.length || !pathIds.every(function(id) { return getObj('path', id); });
        };
        const selected = links.filter(function(link) { return clearScene || isStale(link); });

        selected.forEach(function(link) {
            result.removed++;
            if (!link) return;
            flattenPathIds(link.pathIDs).forEach(function(id) { if (getObj('path', id)) ownedArtifactIds['path:' + id] = true; });
            const controlOwned = link.controlTokID && link.controlTokID !== link.originTokID && getObj('graphic', link.controlTokID);
            const wallOriginOwned = link.aoeType && /wall/i.test(link.aoeType) && getObj('graphic', link.originTokID);
            if (controlOwned) ownedArtifactIds['graphic:' + link.controlTokID] = true;
            if (wallOriginOwned && link.originTokID !== link.controlTokID) ownedArtifactIds['graphic:' + link.originTokID] = true;
        });
        result.restored = Object.keys(ownedArtifactIds).length;
        result.preserved = links.length - selected.length;

        if (!dryRun) {
            selected.slice().forEach(function(link) {
                if (!link) {
                    const emptyIndex = root.links.indexOf(link);
                    if (emptyIndex !== -1) root.links.splice(emptyIndex, 1);
                    return;
                }
                flattenPathIds(link.pathIDs).forEach(function(id) {
                    const path = getObj('path', id);
                    if (path) path.remove();
                });
                modifyTurnTracker('remove', '', '', '', link.controlTokID, '');
                const index = root.links.indexOf(link);
                if (index !== -1) root.links.splice(index, 1);
                const stillUsesControl = root.links.some(function(other) { return other && other.controlTokID === link.controlTokID; });
                if (!stillUsesControl && link.controlTokID !== link.originTokID) removeToken(link.controlTokID);
                const stillUsesWallOrigin = root.links.some(function(other) { return other && other.originTokID === link.originTokID; });
                if (!stillUsesWallOrigin && link.aoeType && /wall/i.test(link.aoeType) && link.originTokID !== link.controlTokID) removeToken(link.originTokID);
            });
        }
        if (result.removed) result.actions.push((clearScene ? 'clear ' : 'prune ') + result.removed + ' SmartAoE link(s) through owner cleanup');
        if (result.restored) result.actions.push('remove ' + result.restored + ' owner-created path/control object(s)');
        if (profile === 'combat') result.preservedDetails.push('valid active scene links (combat only prunes stale links)');
        else if (result.preserved) result.preservedDetails.push('valid active scene links');
        return result;
    };
