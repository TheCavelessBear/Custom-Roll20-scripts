...
const registerEventHandlers = function() {
  ...
  on('destroy:graphic', function(token) {
    if(token) pruneSavedTurnOrders({ dryRun: false, context: { graphicId: token.id } });
  });
};

const pruneSavedTurnOrders = function(options) {
  const records = (state[scriptName] || {}).savedTurnOrders || [];
  const removed = [];
  const dryRun = !!options.dryRun;
  const graphicId = options.context && options.context.graphicId;
  records.slice().forEach(function(record, recordIndex) {
    let order;
    try { order = JSON.parse(record.turnorder); } catch (error) {
      removed.push('savedTurnOrders.' + recordIndex);
      if(!dryRun) records.splice(records.indexOf(record), 1);
      return;
    }
    ...
    const filtered = order.filter(function(entry) { return entry && (entry.id === '-1' || (entry.id !== graphicId && getObj('graphic', entry.id))); });
    if(filtered.length !== order.length) {
      removed.push('savedTurnOrders.' + recordIndex + '.turnorder');
      if(!dryRun) record.turnorder = JSON.stringify(filtered);
    }
  });
...
on("ready",() => {
  checkInstall();
  if(typeof PersistentStateManager !== 'undefined') PersistentStateManager.registerPruner('GroupInitiative', pruneSavedTurnOrders);
