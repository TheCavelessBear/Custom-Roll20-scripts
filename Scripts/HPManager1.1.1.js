...
on('chat:message', function(msg) {
    ...
    const content = hpReplaceInlineRolls(msg);
...
function applyHealing(token, amount, label) {
    ...
    if (isFriendlyHealingTarget(token)) {
        sendChat('HPManager', detailedCard);
        return;
    }
    ...
    sendChat(
        ...
        '{{name=Healing}} ' +
        '{{Target=' + token.get('name') + '}} ' +
        '{{Result=Recovered hit points.}}'
    );

    sendChat('HPManager', '/w gm ' + detailedCard);
}
...
function hpReplaceInlineRolls(msg) {
