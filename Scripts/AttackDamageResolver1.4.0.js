...
on('ready', function() {
    ...
    log('=== AttackDamageResolver 1.4.0 Ready ===');
});
...
on('chat:message', function(msg) {
    ...
    const content = adrReplaceInlineRolls(msg);
    ...
    if (args[1] === 'uncanny') {
        handleUncannyDodge(msg, args);
        return;
    }

    if (args[1] === 'reduce') {
        handleDamageReduction(msg, args);
        return;
    }
    ...
    sendChat('AttackDamageResolver', '/w gm Commands: !adr admin | !adr attack ATTACKER_ID TARGET_ID [--slot SLOT] | !adr apply TARGET_ID [TYPE] [LABEL] [--magic] [--melee] | !adr applyslot SLOT [--magic] [--melee] | !adr targetcmd !command @@target args | !adr targetcmdslot SLOT !command @@target args | !adr fx FX_NAME | !adr fxslot SLOT FX_NAME | !adr ray FX_NAME | !adr rayslot SLOT FX_NAME | !adr slots | !adr clearslots | !adr settarget TARGET_ID | !adr cleartarget | !adr uncanny TOKEN_ID | !adr reduce TOKEN_ID AMOUNT [--source SOURCE_ID] [--range FEET] [--label LABEL] | !adr undo | !adr status');
});
...
function handleAttackRoll(msg, args) {
    ...
    if (penaltyNotes.length) {
        card += ' {{Attack Penalty=' + penaltyNotes.map(adrTemplateValue).join('<br>') + '}}';
    }

    if (reasons.length) {
        card += ' {{Reason=' + reasons.map(adrTemplateValue).join('<br>') + '}}';
    }
...
function handleSetTargetSlots(args) {
    ...
    sendChat(
        ...
        '{{Attacker=' + adrTemplateValue(attacker.get('name')) + '}} ' +
        '{{Slots=' + (lines.length ? lines.map(adrTemplateValue).join('<br>') : 'None') + '}}'
    );
}
...
function showTargetSlots() {
    ...
    sendChat(
        ...
        '{{Slots=' + lines.map(adrTemplateValue).join('<br>') + '}}'
    );
}
...
function showAdrStatus() {
    ...
    sendChat(
        ...
        '/w gm &{template:default} ' +
        '{{name=ADR Status}} ' +
        '{{Remembered Attacker=' + adrTemplateValue(attacker ? attacker.get('name') : 'None') + '}} ' +
        '{{Remembered Target=' + adrTemplateValue(target ? target.get('name') : 'None') + '}} ' +
        '{{Cached Damage=' + adrTemplateValue(cached ? cached.title : 'None') + '}} ' +
        '{{Last Undo Target=' + adrTemplateValue(undoTarget ? undoTarget.get('name') : 'None') + '}} ' +
...
async function handleUncannyDodge(msg, args) {
    const targetId = args[2];
    const undo = state.AttackDamageResolver.lastDamageUndo;

    if (!targetId) {
        sendChat('AttackDamageResolver', '/w "' + msg.who + '" ADR: Uncanny Dodge requires a token ID.');
        return;
    }

    if (!undo) {
        sendChat('AttackDamageResolver', '/w "' + msg.who + '" ADR: There is no recent ADR damage to reduce.');
        return;
    }

    if (undo.targetId !== targetId) {
        sendChat('AttackDamageResolver', '/w "' + msg.who + '" ADR: The most recent ADR damage was applied to a different token.');
        return;
    }

    if (undo.uncannyDodgeApplied || undo.uncannyDodgeInProgress) {
        sendChat('AttackDamageResolver', '/w "' + msg.who + '" ADR: Uncanny Dodge has already been applied to this damage.');
        return;
    }
    ...
    if (!target) {
        sendChat('AttackDamageResolver', '/w "' + msg.who + '" ADR: The damaged token no longer exists.');
        return;
    }

    if (
        String(target.get('bar1_value')) !== String(undo.bar1After) ||
        String(target.get('bar2_value')) !== String(undo.bar2After)
    ) {
        sendChat(
            'AttackDamageResolver',
            '/w "' + msg.who + '" ADR: HP or temporary HP changed after the recorded hit. Uncanny Dodge was not applied.'
        );
        return;
    }

    undo.uncannyDodgeInProgress = true;

    const originalDamage = Math.max(0, parseInt(undo.amount, 10) || 0);
    const reducedDamage = Math.floor(originalDamage / 2);
    const hpAfterFullDamage = target.get('bar1_value');
    const tempAfterFullDamage = target.get('bar2_value');

    try {
        await restoreAdrDamageSnapshot(target, undo);

        const adjustedUndo = await applyDamageToToken(target, reducedDamage) || {
            targetId: target.id,
            amount: reducedDamage,
            bar1Value: undo.bar1Value,
            bar1Max: undo.bar1Max,
            bar2Value: undo.bar2Value,
            bar2Max: undo.bar2Max
        };

        adjustedUndo.bar1After = target.get('bar1_value');
        adjustedUndo.bar2After = target.get('bar2_value');
        adjustedUndo.uncannyDodgeApplied = true;
        adjustedUndo.originalDamage = originalDamage;

        state.AttackDamageResolver.lastDamageUndo = adjustedUndo;

        sendChat(
            'AttackDamageResolver',
            '&{template:default} ' +
            '{{name=' + adrTemplateValue(target.get('name')) + ' — Uncanny Dodge}} ' +
            '{{Reaction=The damage from the most recent attack is halved.}} ' +
            '{{Original Damage=' + originalDamage + '}} ' +
            '{{Damage Taken=' + reducedDamage + '}} ' +
            '{{HP=' + hpAfterFullDamage + ' → ' + target.get('bar1_value') + '}} ' +
            '{{Temp HP=' + tempAfterFullDamage + ' → ' + target.get('bar2_value') + '}}'
        );
    } catch (error) {
        undo.uncannyDodgeInProgress = false;
        sendChat(
            'AttackDamageResolver',
            '/w "' + msg.who + '" ADR: Uncanny Dodge could not restore and reapply the recorded damage.'
        );
    }
}

async function handleDamageReduction(msg, args) {
    const targetId = args[2];
    const reductionExpression = args[3];
    const sourceId = getAdrOptionValue(args, '--source');
    const rangeInput = getAdrOptionValue(args, '--range');
    const label = String(getAdrOptionValue(args, '--label') || 'Damage Reduction').replace(/_/g, ' ');
    const undo = state.AttackDamageResolver.lastDamageUndo;

    if (!targetId || !reductionExpression) {
        sendChat(
            'AttackDamageResolver',
            '/w "' + msg.who + '" ADR: Format: !adr reduce TOKEN_ID AMOUNT [--source SOURCE_ID] [--range FEET] [--label LABEL]'
        );
        return;
    }
    ...
    const target = getObj('graphic', targetId);
    const source = sourceId ? getObj('graphic', sourceId) : null;
    ...
    if (sourceId && !source) {
        sendChat('AttackDamageResolver', '/w "' + msg.who + '" ADR: The damage-reduction source token no longer exists.');
        return;
    }

    if (source && rangeInput !== null) {
        const maximumRange = parseFloat(rangeInput);
        const distance = getTokenEdgeDistanceFeet(source, target);

        if (!Number.isFinite(maximumRange) || maximumRange < 0) {
            sendChat('AttackDamageResolver', '/w "' + msg.who + '" ADR: Damage-reduction range must be a nonnegative number.');
            return;
        }

        if (distance === null || distance > maximumRange) {
            sendChat(
                'AttackDamageResolver',
                '/w "' + msg.who + '" ADR: ' + target.get('name') + ' is outside the ' + maximumRange + '-foot range.'
            );
            return;
        }
    }
    ...
    if (
        ...
        sendChat(
            'AttackDamageResolver',
            '/w "' + msg.who + '" ADR: HP or temporary HP changed after the recorded hit. The damage reduction was not applied.'
        );
        return;
    }

    const reductionRolled = rollDamageReduction(reductionExpression);

    if (reductionRolled === null) {
        sendChat(
            'AttackDamageResolver',
            '/w "' + msg.who + '" ADR: Reduction must be a nonnegative number or a dice expression such as 1d10+3.'
        );
        return;
    }
    ...
    if (originalDamage <= 0) {
        sendChat('AttackDamageResolver', '/w "' + msg.who + '" ADR: The recorded hit has no remaining damage to reduce.');
        return;
    }

    const damagePrevented = Math.min(originalDamage, reductionRolled);
    const reducedDamage = Math.max(0, originalDamage - damagePrevented);
    const hpAfterFullDamage = target.get('bar1_value');
    const tempAfterFullDamage = target.get('bar2_value');
    ...
    try {
        ...
        adjustedUndo.bar1After = target.get('bar1_value');
        adjustedUndo.bar2After = target.get('bar2_value');
        adjustedUndo.damageReductionApplied = true;
        adjustedUndo.originalDamage = originalDamage;
        adjustedUndo.reductionRolled = reductionRolled;
        adjustedUndo.damagePrevented = damagePrevented;
        ...
        sendChat(
            'AttackDamageResolver',
            '&{template:default} ' +
            '{{name=' + (source ? adrTemplateValue(source.get('name')) + ' — ' : '') + adrTemplateValue(label) + '}} ' +
            '{{Reaction=The damage from the most recent hit is reduced.}} ' +
            '{{Target=' + adrTemplateValue(target.get('name')) + '}} ' +
            '{{Reduction Roll=' + reductionRolled + '}} ' +
            '{{Damage Prevented=' + damagePrevented + '}} ' +
            '{{Damage Taken=' + originalDamage + ' → ' + reducedDamage + '}} ' +
            '{{HP=' + hpAfterFullDamage + ' → ' + target.get('bar1_value') + '}} ' +
            '{{Temp HP=' + tempAfterFullDamage + ' → ' + target.get('bar2_value') + '}}'
        );
    } catch (error) {
        sendChat(
            'AttackDamageResolver',
            '/w "' + msg.who + '" ADR: The recorded damage could not be restored and reduced.'
        );
    }
}

function rollDamageReduction(expression) {
    ...
    const modifier = parseInt(dice[3] || '0', 10);
...
function getAdrOptionValue(args, optionName) {
...
async function restoreAdrDamageSnapshot(target, undo) {
    target.set({
        bar1_value: undo.bar1Value,
        bar1_max: undo.bar1Max,
        bar2_value: undo.bar2Value,
    ...
    if (characterId && typeof setSheetItem === 'function') {
        await setSheetItem(characterId, 'hp', undo.bar1Value);

        if (undo.bar1Max !== '') {
            await setSheetItem(characterId, 'hp', undo.bar1Max, 'max');
        }

        await setSheetItem(characterId, 'hp_temp', undo.bar2Value || 0);
        await setSheetItem(characterId, 'hp_temp', undo.bar2Max || 0, 'max');
    }
}
...
async function applyFireShieldRetaliationDamage(attacker, target, retaliation) {
    ...
    const modified = await adrGetAeModifiedDamage(attacker, retaliation.damageType, baseDamage, {
        magical: false,
    ...
    sendChat(
        'AttackDamageResolver',
        '&{template:default} ' +
        '{{name=' + adrTemplateValue(retaliation.label) + ' Retaliation}} ' +
        '{{Shielded Target=' + adrTemplateValue(target.get('name')) + '}} ' +
        '{{Attacker=' + adrTemplateValue(attacker.get('name')) + '}} ' +
        '{{Damage Rolled=' + baseDamage + ' ' + retaliation.damageType + '}} ' +
        '{{Damage Traits=' + adrTemplateValue(modified.note) + '}} ' +
...
async function applyCachedDamage(msg, args) {
    ...
    for (const part of parts) {
        ...
        const modified = await adrGetAeModifiedDamage(target, damageType, part.amount, {
            magical: magical,
            adept: adeptType
        });
    ...
    if (undo) {
        undo.bar1After = target.get('bar1_value');
        undo.bar2After = target.get('bar2_value');
    ...
    if (
        ...
        await ActionEconomyV2API.processDamageResult(lastAttack.attackerId, target.id, hpBefore, hpAfter);
    }
    ...
    sendChat(
        ...
        '/w gm &{template:default} ' +
        '{{name=Damage Applied}} ' +
        '{{Source=' + adrTemplateValue(label) + '}} ' +
        '{{Target=' + adrTemplateValue(target.get('name')) + '}} ' +
        '{{Base Damage=' + parts.map(p => p.amount + ' ' + adrTemplateValue(p.type)).join(' + ') + '}} ' +
        '{{Damage Traits=' + traitNotes.map(adrTemplateValue).join('<br>') + '}} ' +
...
async function adrGetAeModifiedDamage(token, damageType, amount, tags) {
    if (
        typeof ActionEconomyV2API !== 'undefined' &&
        ActionEconomyV2API &&
...
function adrProcessTokenTriggersBar1Change(token, oldHp, newHp) {
    if (
...
async function applyDamageToToken(token, amount) {
    ...
    if (remaining <= 0) {
        undo.bar1After = token.get('bar1_value');
        return undo;
    }
    ...
    const provisionalHp = Math.max(0, hp - remaining);

    token.set('bar1_value', provisionalHp);

    const finalHp = adrProcessTokenTriggersBar1Change(token, hp, provisionalHp);

    token.set('bar1_value', finalHp);

    if (characterId && typeof setSheetItem === 'function') {
        await setSheetItem(characterId, 'hp', finalHp);
    }

    undo.bar1After = finalHp;

    return undo;
}

const adrTemplateValue = function(value) {
...
function adrReplaceInlineRolls(msg) {
...
AttackDamageResolverAPI = {
    maintainState: function(options) {
        options = options || {};
        const dryRun = options.dryRun === true;
        const profile = options.profile || 'prune';
        const root = state.AttackDamageResolver || {};
        const result = { owner: 'AttackDamageResolver', profile: profile, status: 'ok', removed: 0, restored: 0, preserved: 0, actions: [], preservedDetails: [], warnings: [] };
        const targetExists = function(id) { return !!getObj('graphic', id); };
        const hasRecord = function(value) {
            return !!value && (typeof value !== 'object' || Object.keys(value).length > 0);
        };
        const collections = ['attackTargets', 'targetSlots'];
        const singles = ['lastAttack', 'lastDamageUndo', 'lastTurnTokenId'];

        if (profile === 'prune') {
            Object.keys(root.attackTargets || {}).forEach(function(attackerId) {
                if (targetExists(attackerId) && targetExists(root.attackTargets[attackerId])) {
                    result.preserved++;
                    return;
                }
                result.removed++;
                if (!dryRun) delete root.attackTargets[attackerId];
            });
            Object.keys(root.targetSlots || {}).forEach(function(slot) {
                const value = root.targetSlots[slot] || {};
                if (targetExists(value.attackerId) && targetExists(value.targetId)) {
                    result.preserved++;
                    return;
                }
                result.removed++;
                if (!dryRun) delete root.targetSlots[slot];
            });
            if (hasRecord(root.lastAttack)) {
                const lastAttackValid = (!root.lastAttack.attackerId || targetExists(root.lastAttack.attackerId)) &&
                    (!root.lastAttack.targetId || targetExists(root.lastAttack.targetId));
                if (lastAttackValid) result.preserved++;
                else { result.removed++; if (!dryRun) delete root.lastAttack; }
            }
            if (hasRecord(root.lastDamageUndo)) {
                if (root.lastDamageUndo.targetId && targetExists(root.lastDamageUndo.targetId)) result.preserved++;
                else { result.removed++; if (!dryRun) delete root.lastDamageUndo; }
            }
            if (root.lastTurnTokenId) {
                if (targetExists(root.lastTurnTokenId)) result.preserved++;
                else { result.removed++; if (!dryRun) root.lastTurnTokenId = null; }
            }
            if (result.removed) result.actions.push('prune ' + result.removed + ' invalid target, slot, attack, undo, or turn reference(s)');
            if (result.preserved) result.preservedDetails.push('valid attack and undo references');
        } else {
            collections.forEach(function(key) { result.removed += Object.keys(root[key] || {}).length; });
            singles.forEach(function(key) { if (hasRecord(root[key])) result.removed++; });
            const cachedDamageCount = Object.keys(ADR_LAST_DAMAGE).length;
            result.removed += cachedDamageCount;
            if (result.removed) result.actions.push('clear ' + result.removed + ' combat/attack runtime and cached damage record(s)');
            if (cachedDamageCount) result.actions.push('clear ADR_LAST_DAMAGE module cache (' + cachedDamageCount + ')');
            if (!dryRun) {
                root.attackTargets = {};
                root.targetSlots = {};
                root.lastTurnTokenId = null;
                delete root.lastAttack;
                delete root.lastDamageUndo;
                Object.keys(ADR_LAST_DAMAGE).forEach(function(key) { delete ADR_LAST_DAMAGE[key]; });
            }
        }
        return result;
    }
