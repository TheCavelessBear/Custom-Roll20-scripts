...
const SAVE_EFFECTS_REGISTRY = {
    ...
    gutshot: {
        ...
        sourceRequired: true,
        failText: 'The target gains the Paralyzed condition until the end of the source creatureâ€™s next turn.'
    },
...
on('chat:message', function(msg) {
    ...
    const content = seReplaceInlineRolls(msg);
    ...
    if (subcommand === 'forcedmove') {
        handleForcedMovementSave(args);
        return;
    }
...
async function handleLifeDrain(args) {
    ...
    const modifiedDamage = await seGetAeModifiedDamage(
        token,
        'Necrotic',
        damageRoll.total
    );
    ...
    sendChat(
        ...
        '{{Max HP=' + currentMaxHp + ' â†’ ' + newMaxHp + '}} ' +
        '{{HP=' + applied.hpBefore + ' â†’ ' + applied.hpAfter + '}}'
    );
}
...
async function rollAndApplyDamageConditionSave(config) {
    ...
    const saveDamage = getAeSaveDamageResult(
        config.token,
        config.saveKey,
        failed,
        config.successMode,
    ...
    const damageAfterSave = saveDamage.amount;
    const modifiedDamage = await seGetAeModifiedDamage(config.token, config.damageType, damageAfterSave, {
    ...
    sendChat(
        'SaveEffects',
        `/w gm &{template:default} {{name=Damage Condition Save: ${config.conditionDisplay}}} {{Target=${config.token.get('name')}}} {{${config.save} Save=[[${rollFormula}${auraBonus ? ' + ' + auraBonus : ''}]]}} {{DC=${config.dc}}} {{Result=${failed ? 'Failure' : 'Success'}}} {{Damage Roll=${config.damageBreakdown} = ${config.damageTotal} ${config.damageType}}} {{Save Feature=${saveDamage.note}}} {{After Save=${damageAfterSave} ${config.damageType}}} {{Damage Traits=${modifiedDamage.note}}} {{Damage Taken=${damageTaken} ${config.damageType}}} {{HP=${applied.hpBefore} â†’ ${applied.hpAfter}}} {{Temp HP=${applied.tempBefore} â†’ ${applied.tempAfter}}} {{Condition=${failed ? (immune ? 'Immune' : config.conditionDisplay) : 'None'}}}`
    );
    ...
    return {
        ...
        saveDamageFeature: saveDamage.note,
...
function publicDamageResultCard(result) {
    const saveFeature = result.saveDamageFeature && result.saveDamageFeature !== 'None'
        ? '{{Save Feature=' + result.saveDamageFeature + '}} '
        : '';
...
function publicDamageBatchCard(display, results) {
    ...
    const effects = results.map(function(result) {
        const saveFeature = result.saveDamageFeature && result.saveDamageFeature !== 'None'
            ? ' â€” ' + result.saveDamageFeature
            : '';

        return result.targetName + ': ' + result.damageTaken + ' ' + result.damageType + ' damage' + saveFeature;
    });
...
function getAeSaveDamageResult(token, saveKey, failed, successMode, damageTotal) {
    if (
        typeof ActionEconomyV2API !== 'undefined' &&
        ActionEconomyV2API &&
        typeof ActionEconomyV2API.getSaveDamageResult === 'function'
    ) {
        return ActionEconomyV2API.getSaveDamageResult(
            token,
            saveKey,
            failed,
            successMode,
    ...
    return {
        amount: failed ? damageTotal : getSuccessDamage(damageTotal, successMode),
        note: 'None'
    };
}

async function seGetAeModifiedDamage(token, damageType, amount, tags) {
    if (
        typeof ActionEconomyV2API !== 'undefined' &&
        ActionEconomyV2API &&
...
function seProcessTokenTriggersBar1Change(token, oldHp, newHp) {
    if (
...
async function seApplyDamageToToken(token, amount) {
    ...
    if (remaining > 0 && !isNaN(hpBefore)) {
        ...
        hpAfter = seProcessTokenTriggersBar1Change(token, hpBefore, hpAfter);
...
async function handleForcedMovementSave(args) {
    const saveKey = args[2];
    const targetId = args[3];
    const dcInput = args[4];
    const sourceTokenId = getOptionValue(args, '--source');
    const direction = getOptionValue(args, '--direction');
    const distance = parseFloat(getOptionValue(args, '--distance'));
    const display = String(getOptionValue(args, '--name') || 'Forced Movement').replace(/_/g, ' ');
    const saveName = SAVE_ABILITY_NAMES[saveKey];
    const target = getObj('graphic', targetId);
    const source = getObj('graphic', sourceTokenId);

    if (!saveName || !target || !source || !direction || !Number.isFinite(distance) || distance <= 0) {
        sendChat(
            'SaveEffects',
            '/w gm Format: !se forcedmove SAVE TARGET_ID DC --source SOURCE_ID --direction toward/away/DIRECTION --distance FEET [--name LABEL]'
        );
        return;
    }
    ...
    const saveBonus = await getSaveBonus(target, saveKey);

    if (saveBonus === null) {
        sendChat('SaveEffects', '/w gm Could not find target save bonus for ' + saveName + ' on ' + target.get('name') + '.');
        return;
    }

    const rollMode = getAeSaveRollMode(target, saveKey, 'forcedmovement');
    const first = randomInteger(20);
    const second = rollMode === 'normal' ? null : randomInteger(20);
    let d20 = first;
    ...
    const auraBonus = getAeSaveBonusModifier(target, saveKey);
    const total = d20 + saveBonus + auraBonus;
    const failed = total < dc;
    let movementResult = null;
    let effectText = 'No movement.';

    if (failed) {
        if (
            typeof ActionEconomyV2API === 'undefined' ||
            !ActionEconomyV2API ||
            typeof ActionEconomyV2API.forceMove !== 'function'
        ) {
            sendChat('SaveEffects', '/w gm ActionEconomyV2 forceMove API is unavailable.');
            effectText = 'The save failed, but forced movement could not be applied.';
        }
        else {
            movementResult = ActionEconomyV2API.forceMove(
                sourceTokenId,
                targetId,
                direction,
                distance
            ...
            effectText = movementResult && movementResult.success
                ? 'Moved ' + distance + ' feet: ' + movementResult.movementLabel + '.'
                : 'The save failed, but forced movement could not be applied.';

            if (!movementResult || !movementResult.success) {
                sendChat(
                    'SaveEffects',
                    '/w gm Forced movement failed: ' + (movementResult ? movementResult.message : 'Unknown error.')
                );
            }
        }
    }
    ...
    sendChat(
        ...
        '&{template:default} ' +
        '{{name=' + display + '}} ' +
        '{{Result=' + target.get('name') + ' ' + (failed ? 'failed' : 'saved') + ' on their ' + saveName + ' Saving Throw.}} ' +
        '{{Effect=' + effectText + '}}'
    );
}
...
function seReplaceInlineRolls(msg) {
...
SaveEffectsAPI.maintainState = function(options) {
    options = options || {};
    const dryRun = options.dryRun === true;
    const profile = options.profile || 'prune';
    const root = state.SaveEffects || {};
    const sources = root.sources || {};
    const result = { owner: 'SaveEffects', profile: profile, status: 'ok', removed: 0, restored: 0, preserved: 0, actions: [], preservedDetails: [], warnings: [] };

    if (profile === 'prune') {
        Object.keys(sources).forEach(function(playerId) {
            if (getObj('graphic', sources[playerId])) {
                result.preserved++;
                return;
            }
            result.removed++;
            if (!dryRun) delete sources[playerId];
        });
        if (result.removed) result.actions.push('prune ' + result.removed + ' missing source-token reference(s)');
        if (result.preserved) result.preservedDetails.push('valid remembered source tokens');
    } else {
        result.removed = Object.keys(sources).length;
        if (result.removed) result.actions.push('clear ' + result.removed + ' remembered source-token record(s)');
        if (!dryRun) root.sources = {};
    }
    return result;
};
