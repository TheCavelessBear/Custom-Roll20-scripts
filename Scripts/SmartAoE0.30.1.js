const SmartAoE = (() => {
    ...
    */
    ...
    const pruneSmartAoEState = function(options) {
        options = options || {};
        const links = (state[scriptName] || {}).links || [];
        const removed = [];
        const dryRun = !!options.dryRun;
        const context = options.context || {};
        links.slice().forEach(function(link, index) {
            const pathIds = link && Array.isArray(link.pathIDs) ? link.pathIDs : [];
            const invalid = !link || link.controlTokID === context.graphicId || link.originTokID === context.graphicId || link.pageID === context.pageId || pathIds.indexOf(context.pathId) !== -1 || !getObj('graphic', link.controlTokID) || (link.originTokID && !getObj('graphic', link.originTokID)) || (link.pageID && !getObj('page', link.pageID)) || pathIds.some(function(id) { return !getObj('path', id); });
            if (invalid) { removed.push('links.' + index); if (!dryRun) links.splice(links.indexOf(link), 1); }
        });
    ...
    const registerEventHandlers = function() {
        ...
        on('destroy:graphic', function(token) {
            smartAoE_handleRemoveToken(token);
            if (token) pruneSmartAoEState({ dryRun: false, context: { graphicId: token.id } });
        });
        on('destroy:path', function(path) { if (path) pruneSmartAoEState({ dryRun: false, context: { pathId: path.id } }); });
        on('destroy:page', function(page) { if (page) pruneSmartAoEState({ dryRun: false, context: { pageId: page.id } }); });
    };
 
    on("ready",() => {
        checkInstall();
        if (typeof PersistentStateManager !== 'undefined') PersistentStateManager.registerPruner('SmartAoE', pruneSmartAoEState);
