...
const SpawnDefaultToken = (() => {
    ...
    function spawnTokenAtXY (who, tokenJSON, pageID, spawnLayer, spawnX, spawnY, currentSideNew, sizeX, sizeY, zOrder, lightRad, lightDim, mook, UDL, bar1Val, bar1Max, bar1Link, bar2Val, bar2Max, bar2Link, bar3Val, bar3Max, bar3Link, expandIterations, expandDelay, destroyWhenDone, angle, isDrawing, tokenName, tooltip, tokenPropValPairs, bringSourceTokenToFront) {
        ...
        try {
            ...
            // Force Roll20's lighting editor to synchronize the spawned token's
            // dim-light distance by making one genuine post-creation change.
            if (spawnObj && UDL && baseObj.emits_low_light) {
                const intendedLowLightDistance = parseFloat(baseObj.low_light_distance);
                
                if (Number.isFinite(intendedLowLightDistance) && intendedLowLightDistance > 0) {
                    const spawnedTokenId = spawnObj.id;
                    const temporaryLowLightDistance =
                        intendedLowLightDistance > 1
                            ? intendedLowLightDistance - 1
                            : intendedLowLightDistance + 1;
                    
                    setTimeout(() => {
                        const spawnedToken = getObj('graphic', spawnedTokenId);
                        
                        if (spawnedToken) {
                            spawnedToken.set(
                                'low_light_distance',
                                temporaryLowLightDistance.toString()
                            );
                        }
                    }, 100);
                    ...
                    setTimeout(() => {
                        ...
                        if (spawnedToken) {
                            spawnedToken.set(
                                'low_light_distance',
                                intendedLowLightDistance.toString()
                            );
                        }
                    }, 250);
                }
