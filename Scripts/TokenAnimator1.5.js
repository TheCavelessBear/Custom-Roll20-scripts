...
const TokenAnimator = (() => {
    ...
    return {
        ...
        maintainState: function(options) {
            ...
            const root = state.TokenAnimator || {};
            const baselines = root.tokens || {};
            const result = { owner: SCRIPT, profile: profile, status: 'ok', removed: 0, restored: 0, preserved: 0, actions: [], preservedDetails: [], warnings: [] };
            const resetBaselines = profile === 'campaign' || profile === 'all';
            const baselineIds = Object.keys(baselines);
            const staleIds = baselineIds.filter(function(tokenId) {
                return !getObj('graphic', tokenId) || !isValidBaseline(baselines[tokenId]);
            });

            if (resetBaselines) {
                const animationIds = Object.keys(activeAnimations);
                result.removed = baselineIds.length + animationIds.length;
                if (baselineIds.length) result.actions.push('clear ' + baselineIds.length + ' stored token baseline(s)');
                if (animationIds.length) result.actions.push('cancel ' + animationIds.length + ' active animation(s)');
                if (!dryRun) {
                    animationIds.forEach(function(tokenId) { cancelAnimation(tokenId); });
                    root.tokens = {};
                    if (state.TokenSizeAnimator && state.TokenSizeAnimator.tokens) state.TokenSizeAnimator.tokens = {};
                }
            } else {
                result.removed = staleIds.length;
                result.preserved = baselineIds.length - staleIds.length;
                if (!dryRun) staleIds.forEach(function(tokenId) { delete baselines[tokenId]; });
                if (staleIds.length) result.actions.push('prune ' + staleIds.length + ' missing/invalid token baseline(s)');
                if (result.preserved) result.preservedDetails.push('valid token baselines and active animation utility state');
            }
            return result;
        }
    };
