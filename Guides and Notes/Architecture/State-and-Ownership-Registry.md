...
| 17 | `SmartAoE0.31.js` | Computed `state.SmartAoE`: schema/version and AoE link records | Configured apply actions may alter token properties; no fixed bar ownership or Beacon calls | Owns AoE geometry/link state. |
| 18 | `DoorSounds1.1.0.js` | `state.DoorSounds`: groups, doors, config, initialized flag, version | None | Owns door-to-sound registry. |
...
| 22 | `ActionEconomyV2.9.0.js` | `state.ActionEconomyV2`: economy, movement, attacks, conditions/effects, mounts, modifiers, saves, hazards, summons, visuals, terrain, sizes, damage sources, pending records | Reads/writes bars 1–2 for owned support/fallback paths; owns bar 3; Beacon helpers plus explicit `hp`, `hp_temp`, `spell_save_dc`, `speed`, `initiative_bonus`, and `user.*` values | Owns all AE mechanics, not general damage application. Dark One’s Blessing uses asynchronous total `level` and `charisma_mod` reads. |
| 23 | `ZeroFrame.js` | `state.ZeroFrame`: schema/config/loop order plus `state.torii` | None | Owns meta-operation orchestration state. |
| 24 | `SaveEffects1.4.0.js` | `state.SaveEffects.sources`, keyed by source/player context | Save damage reads/writes bars 1–2 and HP max; after the TokenTriggers resolution, it writes the returned Bar 1 result and represented Beacon `hp`; Beacon `hp_temp`, HP max, and generic async save/skill reads | Owns saves and save-based damage, not AE effects. |
...
| 32 | `AoEBoom1.2.0.js` | `state.AoEBoom.templates`, keyed by template/path identity | No bar ownership; reads Beacon `spell_save_dc` | Owns AoE geometry/template state. |
...
| 36 | `StateWipe2.0.0.js` | No owned state; inventories every registered suite root and orchestrates owner maintenance; factory `all` additionally deletes shared `state.torii` and obsolete migration roots | No direct bars/Beacon; mechanically consequential cleanup remains with each owner | Administrative maintenance orchestrator, not a migration mechanism. Never deletes `state.TheAaron` or unrecognized roots. |
...
| 39 | `TokenTriggers1.4.0.js` | `state.TokenTriggers`: character configurations, token runtime, version, last active token; short-lived transition deduplication is nonpersistent | Owns threshold reactions/presentation on bar 1, including HP-zero restore and Relentless setting HP to 1; no direct Beacon calls | Owns reactions and explicit/native transition deduplication, not damage. ADR/SE pass only token and old/new HP through the generic API. |
| 40 | `TokenAnimator1.5.js` | `state.TokenAnimator.tokens[tokenId]` baselines/version; migrates legacy `state.TokenSizeAnimator.tokens`; active combined-animation records remain transient | No bars/Beacon | Owns animation baseline/lifecycle. |
...
| 43 | `LootManager1.5.js` | `state.LootManager`: config/version/character keyrings; loot serialized in token GM notes; take/consume locks remain transient | No owned bars; Beacon `gp` read/write and `sleight_of_hand_bonus` read | Owns loot, ordinary-item consumption, keys, containers, and currency workflow. |

## Cross-Script State Rules

### StateWipe 2.0 Maintenance Contract

`StateWipe2.0.0.js` orchestrates GM-only `!statewipe prune|combat|scene|setting|campaign|all` and `!statelist`. Every profile first invokes the documented owner `maintainState({ profile, dryRun })` API and shows a dry-run card; selective profiles require literal `WIPE`, while factory `all` requires `WIPE ALL`. Owner summaries are structured as `{ owner, profile, status, removed, restored, preserved, actions, preservedDetails, warnings }`.

`ActionEconomyV2API`, `AttackDamageResolverAPI`, `SaveEffectsAPI`, `AoEBoom`, `SmartAoE`, `TokenTriggers`, `TokenAnimator`, `LootManager`, and `DoorSounds` own their own maintenance/reversal work. StateWipe directly prunes only disposable Executioner, empty HPManager, and BeaconAttributeTester records, and calls `MapChange.ConstructMaps` to rebuild generated map indexes. `all` calls owners first then removes every registered suite root plus shared `state.torii` and legacy `state.TokenSizeAnimator`/`state.AuraToggle`; it never removes `state.TheAaron` or unrecognized state.

Profile boundaries:

- `prune` removes only invalid references; `combat` adds combat/attack runtime cleanup; `scene` adds owner-created encounter objects; `setting` reverses remaining transient mechanics while retaining durable PC/Ally setup; `campaign` resets gameplay/progression while retaining installed-tool configuration; `all` performs owner reversal and then deletes every registered suite root.
- Framework/config roots (`GroupInitiative`, `simpleSound`, `TokenMod`, `ScriptCards`, the meta-toolchain roots including shared `torii`, `TurnMarker`, `DoorSounds`, and `MapChange`) survive every selective profile. DoorSounds only prunes deleted-door assignments and MapChange only rebuilds generated indexes.
- Mixed roots (`ActionEconomyV2`, `SmartAoE`, `TokenTriggers`, and `LootManager`) are never blindly deleted by a selective profile. A failed or unprovable mechanical reversal remains recorded and is returned as a warning.
