...
| 40 | `TokenAnimator1.4.js` | `state.TokenAnimator.tokens[tokenId]` baselines/version; migrates legacy `state.TokenSizeAnimator.tokens`; active combined-animation records remain transient | No bars/Beacon | Owns animation baseline/lifecycle. |
...
| 43 | `LootManager1.4.js` | `state.LootManager`: config/version/character keyrings; loot serialized in token GM notes; take/consume locks remain transient | No owned bars; Beacon `gp` read/write and `sleight_of_hand_bonus` read | Owns loot, ordinary-item consumption, keys, containers, and currency workflow. |
