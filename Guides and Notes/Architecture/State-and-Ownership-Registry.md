...
| 39 | `TokenTriggers1.3.2.js` | `state.TokenTriggers`: character configurations, token runtime, version, last active token | Owns threshold reactions/presentation on bar 1, including HP-zero restore and Relentless setting HP to 1; no direct Beacon calls | Owns reactions, not damage. API alias discrepancy requires verification. |
